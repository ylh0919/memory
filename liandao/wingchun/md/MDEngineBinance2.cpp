...
static MDEngineBinance2* g_md_binance = nullptr;
...
MDEngineBinance2::MDEngineBinance2(): IMDEngine(SOURCE_BINANCE2)
{
    logger = yijinjing::KfLog::getLogger("MdEngine.Binance2");
    timer[0] = getTimestamp();/*quest3 edited by fxw*/
    timer[1] = timer[2] = timer[0];/*quest3 edited by fxw*/
}

void MDEngineBinance2::load(const json& j_config)
{
    ...
    if(coinPairWhiteList.Size() == 0) {
        KF_LOG_ERROR(logger, "MDEngineBinance2::lws_write_subscribe: subscribeCoinBaseQuote is empty. please add whiteLists in kungfu.json like this :");
    ...
    KF_LOG_INFO(logger, "MDEngineBinance2::load:  book_depth_count: "
		<< book_depth_count << " trade_count: " << trade_count << " rest_get_interval_ms: " << rest_get_interval_ms); 	
	//write_errormsg(160,"test md binance");
}

void MDEngineBinance2::connect(long timeout_nsec)
{
   KF_LOG_INFO(logger, "MDEngineBinance2::connect:"); 	
	
   connected = true;
}

void MDEngineBinance2::login(long timeout_nsec)
{
	g_md_binance = this;
    ...
    while(map_itr != coinPairWhiteList.GetKeyIsStrategyCoinpairWhiteList().end()) {
   	...
   	KF_LOG_INFO(logger, "MDEngineBinance2::login:"); 	

   	logged_in = true;
}

void MDEngineBinance2::connect_lws(std::string symbol, lws_event e)
{
		std::string t = symbol;
...
void MDEngineBinance2::on_lws_data(struct lws* conn, const char* data, size_t len)
{
...
void MDEngineBinance2::on_lws_connection_error(struct lws* conn)
{
	auto iter = lws_handle_map.find(conn);
	if(iter != lws_handle_map.end())
	{
...
void MDEngineBinance2::on_lws_market_trade(const char* data, size_t len)
{
    ...
    if(ticker.length() == 0) {
        KF_LOG_INFO(logger, "MDEngineBinance2::on_lws_market_trade: not in WhiteList , ignore it:" << symbol);
        return;
    }
...
void MDEngineBinance2::on_lws_book_update(const char* data, size_t len, const std::string& ticker)
{
    ...
    if(has_update)
    {
        ...
        if(strategy_ticker.length() == 0) {
            KF_LOG_INFO(logger, "MDEngineBinance2::on_lws_market_trade: not in WhiteList , ignore it:" << strategy_ticker);
            return;
        }
		...
		std::unique_lock<std::mutex> lck1(book_mutex);
		auto it = control_orderbook_map.find(strategy_ticker);
		if(it != control_orderbook_map.end()){
			it->second = getTimestamp();
		}
		lck1.unlock();
...
void MDEngineBinance2::on_lws_kline(const char* src, size_t len)
{
...
void MDEngineBinance2::set_reader_thread()
{
	IMDEngine::set_reader_thread();

   	rest_thread = ThreadPtr(new std::thread(boost::bind(&MDEngineBinance2::loop, this)));
}

void MDEngineBinance2::logout()
{
	lws_context_destroy( context );
   	KF_LOG_INFO(logger, "MDEngineBinance2::logout:"); 	
}

void MDEngineBinance2::release_api()
{
   KF_LOG_INFO(logger, "MDEngineBinance2::release_api:"); 	
}

void MDEngineBinance2::subscribeMarketData(const vector<string>& instruments, const vector<string>& markets)
{
	/* Connect if we are not connected to the server. */
   	KF_LOG_INFO(logger, "MDEngineBinance2::subscribeMarketData:"); 	
}

void MDEngineBinance2::GetAndHandleDepthResponse(const std::string& symbol, int limit) 
{
...
void MDEngineBinance2::GetAndHandleTradeResponse(const std::string& symbol, int limit)
{
...
void MDEngineBinance2::loop()
{
		while(isRunning)
		{
            ...
            {
            	errorId = 116;
            	errorMsg = "kline max refresh wait time exceeded";
...
/*quest3 edited by fxw,starts here*/
int MDEngineBinance2::Get_refresh_normal_check_book_s()
{
    return refresh_normal_check_book_s;
}

int MDEngineBinance2::Get_refresh_normal_check_kline_s()
{
    return refresh_normal_check_kline_s;
}

inline int64_t MDEngineBinance2::getTimestamp()
{   /*返回的是毫秒*/
...
BOOST_PYTHON_MODULE(libbinance2md)
{
    using namespace boost::python;
    class_<MDEngineBinance2, boost::shared_ptr<MDEngineBinance2> >("Engine")
    .def(init<>())
    .def("init", &MDEngineBinance2::initialize)
    .def("start", &MDEngineBinance2::start)
    .def("stop", &MDEngineBinance2::stop)
    .def("logout", &MDEngineBinance2::logout)
    .def("wait_for_stop", &MDEngineBinance2::wait_for_stop);
}
