...
static MDEngineBinanceD* g_md_binanced = nullptr;

static int lws_event_cb(struct lws* wsi, enum lws_callback_reasons reason, void* user, void* in, size_t len)
{
	std::cout << "MdEngine.BinanceD: lws_event_cb (lws_callback_reasons)" << reason << std::endl;
	switch (reason)
	{
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		if (g_md_binanced)
		{
			g_md_binanced->on_lws_data((const char*)in, len);
		}
		break;
	}
	...
	case LWS_CALLBACK_CLIENT_CLOSED:
		/*
		{
			std::cout << "MdEngine.BinanceD LWS_CALLBACK_CLOSED or LWS_CALLBACK_CLIENT_CONNECTION_ERROR" << std::endl;
			if(g_md_binanced)
			{
				g_md_binanced->on_lws_connection_error(wsi);
			}
			break;
		}
		*/
	case LWS_CALLBACK_WSI_DESTROY: //先CLOSED再DESTROY
	{
		if (g_md_binanced)
		{
			g_md_binanced->on_lws_connection_destroy(wsi, reason);
		}
		break;
	}
	default:
...
MDEngineBinanceD::MDEngineBinanceD() : IMDEngine(SOURCE_BINANCED)
{
	logger = yijinjing::KfLog::getLogger("MdEngine.BinanceD");
	timer[0] = getTimestamp();
	timer[1] = timer[2] = timer[0];
}

void MDEngineBinanceD::load(const json& j_config)
{
	...
	*/
	...
	if (coinPairWhiteList.Size() == 0) {
		KF_LOG_ERROR(logger, "MDEngineBinanceD::lws_write_subscribe: subscribeCoinBaseQuote is empty. please add whiteLists in kungfu.json like this :");
	...
	KF_LOG_INFO(logger, "MDEngineBinanceD::load:  book_depth_count: "
...
void MDEngineBinanceD::connect(long timeout_nsec)
{
	KF_LOG_INFO(logger, "MDEngineBinanceD::connect:");

	connected = true;
}

void MDEngineBinanceD::login(long timeout_nsec)
{
	g_md_binanced = this;
	...
	//it's path for binancef, not used in binanced
	std::string path = "";
	//used in binanced
	std::string path_d = "";
	while (true) {
	...
	//connect_lws(path);
	KF_LOG_INFO(logger, "path,not used in binanced " << path);
	KF_LOG_INFO(logger, "path_d " << path_d);
	connect_lws(path_d, true);

	KF_LOG_INFO(logger, "MDEngineBinanceD::login:");

	logged_in = true;
	is_logout = false;
}

bool MDEngineBinanceD::is_coin_m_futures(std::string symbol)
...
void MDEngineBinanceD::get_symbol_and_event(std::string temp, std::vector<pair>& vec)
{
	KF_LOG_INFO(logger, "MDEngineBinanceD::get_symbol_and_event");
	int pos = 1, pos1, pos2;
	...
	KF_LOG_INFO(logger, "MDEngineBinanceD::get_symbol_and_event end");
}

//mofified, is_coin_m decided which api connectting
void MDEngineBinanceD::connect_lws(std::string temp, bool is_coin_m)
{
	KF_LOG_INFO(logger, "MDEngineBinanceD::connect_lws");
...
void MDEngineBinanceD::on_lws_data(const char* data, size_t len)
{
...
//fixed
void MDEngineBinanceD::on_lws_connection_error(struct lws* conn)
{
...
void MDEngineBinanceD::on_lws_connection_destroy(lws* conn, lws_callback_reasons reason)
{
...
void MDEngineBinanceD::on_lws_funding(const char* data, size_t len)
{
	...
	if (ticker.empty())
	{
		KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_funding: not in WhiteList , ignore it:" << symbol);
		return;
	}
...
void MDEngineBinanceD::on_lws_market_trade(const char* data, size_t len)
{
	...
	KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_market_trade:" << data);
	Document d;
	d.Parse(data);
	...
	std::string ticker = coinPairWhiteList.GetKeyByValue(symbol);
	if (ticker.length() == 0) {
		KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_market_trade: not in WhiteList , ignore it:" << symbol);
		return;
	}
	...
	if (need_another_kline) {
		KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_market_trade: writing klinedata" << symbol);
		...
		lock.unlock();
		KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_market_trade: writing klinedata end" << symbol);
	}
	//edit end

	on_trade(&trade);

	KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_market_trade: processed successfully");
}

void MDEngineBinanceD::on_lws_book_update(const char* data, size_t len)
{
	KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_book_update:" << data);
	Document d;
	d.Parse(data);
	...
	if (has_update)
	{
		...
		if (strategy_ticker.length() == 0) {
			KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_book_update: not in WhiteList , ignore it:" << symbol);
			return;
		}
		...
		//if (timeNow - md.UpdateMicroSecond > orderbook_latency_threshold_exceeded_ms) {
	...
	KF_LOG_INFO(logger, "MDEngineBinanceD::on_lws_book_update: processed successfully");
}

void MDEngineBinanceD::on_lws_kline(const char* src, size_t len)
{
...
void MDEngineBinanceD::set_reader_thread()
{
	IMDEngine::set_reader_thread();

	ws_thread = ThreadPtr(new std::thread(boost::bind(&MDEngineBinanceD::loop, this)));
	if (need_get_snapshot_via_rest)
		rest_thread = ThreadPtr(new std::thread(boost::bind(&MDEngineBinanceD::rest_loop, this)));
	else
		KF_LOG_INFO(logger, "MDEngineBinanceD::set_reader_thread, won't start rest_loop");
	//edit here
	if (need_another_kline)
		kline_thread = ThreadPtr(new std::thread(boost::bind(&MDEngineBinanceD::klineloop, this)));
}

//edit begin
//int bar_duration_s = 60;
//int bar_duration_s = 5;
void MDEngineBinanceD::klineloop()
{
...
void MDEngineBinanceD::control_kline(bool first_kline, int64_t time)
{
...
void MDEngineBinanceD::handle_kline(int64_t time)
{
	...
	for (it = kline_hand_map.begin(); it != kline_hand_map.end(); it++) {
		...
		strcpy(market.ExchangeID, "binanced");
...
std::string MDEngineBinanceD::gettime(int64_t time)
{
	time_t now = time;
...
std::string MDEngineBinanceD::dealzero(std::string time)
{
	if (time.length() == 1) {
		time = "0" + time;
	}
...
void MDEngineBinanceD::get_kline_via_rest(string symbol, string interval, bool ignore_startTime, int64_t startTime, int64_t endTime, int limit)
{
	std::string ticker = coinPairWhiteList.GetKeyByValue(symbol);
	if (ticker.empty())
	{
...
string MDEngineBinanceD::getIntervalStr(int64_t sec)
{
	switch (sec) {
	case 60:
		return "1m";
...
std::string MDEngineBinanceD::getdate(int64_t time)
{
	time_t now = time;
	tm* ltm = localtime(&now);
...
void MDEngineBinanceD::logout()
{
	is_logout = true;
	lws_context_destroy(context);
	KF_LOG_INFO(logger, "MDEngineBinanceD::logout:");
}

void MDEngineBinanceD::release_api()
{
	KF_LOG_INFO(logger, "MDEngineBinanceD::release_api:");
}

void MDEngineBinanceD::subscribeMarketData(const vector<string>& instruments, const vector<string>& markets)
{
	/* Connect if we are not connected to the server. */
	KF_LOG_INFO(logger, "MDEngineBinanceD::subscribeMarketData:");
}

void MDEngineBinanceD::get_snapshot_via_rest()
{
	//while (isRunning)
	//{
	std::unordered_map<std::string, std::string>::iterator map_itr;
...
void MDEngineBinanceD::check_snapshot()
{
...
void MDEngineBinanceD::rest_loop()
{
	while (isRunning)
	{
		int64_t now = getTimestamp();
...
inline int64_t MDEngineBinanceD::getTimestamp()
{   /*返回的是毫秒*/
	long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
	return timestamp;
}

BOOST_PYTHON_MODULE(libbinancedmd)
{
	using namespace boost::python;
	class_<MDEngineBinanceD, boost::shared_ptr<MDEngineBinanceD> >("Engine")
		.def(init<>())
		.def("init", &MDEngineBinanceD::initialize)
		.def("start", &MDEngineBinanceD::start)
		.def("stop", &MDEngineBinanceD::stop)
		.def("logout", &MDEngineBinanceD::logout)
		.def("wait_for_stop", &MDEngineBinanceD::wait_for_stop);
}