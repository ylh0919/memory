...
void MDEngineEmx::onTrade(Document& json)
{
        ...
        strcpy(trade.TradeTime,last_trade["logical_time"].GetString());
...
// }
	    ...
	    if (priceBook20Assembler.Assembler(ticker, md)) {
            strcpy(md.ExchangeID, "emx");
            KF_LOG_INFO(logger, "MDEngineEmx::onDepth: on_price_book_update");
            /*edited by zyy,starts here*/
            timer = getTimestamp();
            if(md.BidLevelCount < level_threshold || md.AskLevelCount < level_threshold) {
                KF_LOG_INFO(logger, "MDEngineEmx::onBook: failed,level count < level threshold :"<<md.BidLevelCount<<" "<<md.AskLevelCount<<" "<<level_threshold);
            ...
            KF_LOG_INFO(logger, "MDEngineEmx::onBook: on_price_book_update error");
        }    
