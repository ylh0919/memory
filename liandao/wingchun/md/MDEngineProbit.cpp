...
void MDEngineProbit::onMarketData(Document& json)
{
	...
	if(json.HasMember("order_books_l2") && json["order_books_l2"].IsArray()) 
	{
        auto order_book = json["order_books_l2"].GetArray();
        int len = json["order_books_l2"].Size();
        for(int i = len-1 ; i >= 0; i--)
        {
            std::string ob_side="";
            std::string ob_price="";
            std::string ob_quantity="";
            //auto& order_book = json["order_books_l2"].GetArray()[i];
            if(order_book[i].IsObject())
            {
                if(order_book[i].HasMember("side") && order_book[i]["side"].IsString())
                {
                    ob_side = order_book[i]["side"].GetString();
                }
                if(order_book[i].HasMember("price") && order_book[i]["price"].IsString())
                {
                    ob_price = order_book[i]["price"].GetString();
                }
                if(order_book[i].HasMember("quantity") && order_book[i]["quantity"].IsString())
                {
                    ob_quantity = order_book[i]["quantity"].GetString();
                }
                ...
                if (reset)
                {
                    ...
                    if(strcmp(ob_side.c_str(), "buy") == 0)
                    {
                        ...
                        //KF_LOG_INFO(logger,"UpdateBidPrice ticker="<<ticker<<"price="<<price<<"volume="<<volume);
                    ...
                    {
                        priceBook20Assembler.UpdateAskPrice(ticker, price, volume);
                        //KF_LOG_INFO(logger,"UpdateAskPrice ticker="<<ticker<<"price="<<price<<"volume="<<volume);
                ...
                else
                {
                    ...
                    if(strcmp(ob_side.c_str(), "sell") == 0)
                    {
                        if (volume == 0)
                        {
                            //--buy_Cnt;
                    ...
                    else if(strcmp(ob_side.c_str(), "buy") == 0)
                    {
                        if (volume == 0)
                        {
                            //--sell_Cnt;
