#ifndef PROJECT_TDENGINEBINANCEF_H
#define PROJECT_TDENGINEBINANCEF_H
...
struct PendingBinanceFOrderStatus
{
...
struct PendingBinanceFTradeStatus
{
    char_31 InstrumentID;   //合约代码
...
struct AccountUnitBinanceF
{
    ...
    std::vector<PendingBinanceFOrderStatus> newOrderStatus;
    std::vector<PendingBinanceFOrderStatus> pendingOrderStatus;
    std::vector<PendingBinanceFTradeStatus> newTradeStatus;
    std::vector<PendingBinanceFTradeStatus> pendingTradeStatus;
    ...
    AccountUnitBinanceF(const AccountUnitBinanceF& source);
    AccountUnitBinanceF();
    ~AccountUnitBinanceF();
};
 ...
 * CTP trade engine
 */
class TDEngineBinanceF: public ITDEngine
{
public:
    ...
    virtual string name() const { return "TDEngineBinanceF"; };
    ...
    void action_order_thread(AccountUnitBinanceF* unit, string ticker, const LFOrderActionField data, std::string account_type, int requestId);
    void batch_cancel_order_thread(AccountUnitBinanceF* unit, string ticker, const LFBatchCancelOrderField* data, std::string account_type, int requestId);

    //websocket
    void on_lws_data(struct lws* conn, const char* data, size_t len);
    void on_lws_connection_error(struct lws* conn);
    int lws_write_subscribe(struct lws* conn);
    bool lws_login(AccountUnitBinanceF& unit, long timeout_nsec);
...
private:
    ...
    vector<AccountUnitBinanceF> account_units;
    ...
    void withdrawl_currency(string asset, string address, string addresstag, string amount, Document& json, AccountUnitBinanceF& unit);
    bool loadExchangeOrderFilters(AccountUnitBinanceF& unit, Document &doc);
    inline void onRspNewOrderRESULT(const LFInputOrderField* data, AccountUnitBinanceF& unit, Document& result, int requestId);
    inline void onRspNewOrderFULL(const LFInputOrderField* data, AccountUnitBinanceF& unit, Document& result, int requestId);
    void onRtnNewOrder(const LFInputOrderField* data, AccountUnitBinanceF& unit, int requestId,string remoteOrderId,int64_t fixedVolume,int64_t fixedPrice);
    ...
    int64_t getTimeDiffOfExchange(AccountUnitBinanceF& unit);
    std::string parseJsonToString(Document &d);
private:
    int HTTP_RESPONSE_OK = 200;
    void send_order(AccountUnitBinanceF& unit, const char *symbol, const char *side, const char *type,
    ...
    void get_order(AccountUnitBinanceF& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json);
    void cancel_order(AccountUnitBinanceF& unit, const char *symbol, long orderId, const char *origClientOrderId, const char *newClientOrderId, Document &doc);
    ...
    void batch_cancel_orders(AccountUnitBinanceF& unit, const char *symbol, std::vector<long> orderIdList, std::vector<std::string> origClientOrderIdList, Document& json);
    void get_my_trades(AccountUnitBinanceF& unit, const char *symbol, int limit, int64_t fromId, Document &doc);
    void get_open_orders(AccountUnitBinanceF& unit, const char *symbol, Document &doc);
    void get_exchange_infos(AccountUnitBinanceF& unit, Document &doc);
    void get_exchange_time(AccountUnitBinanceF& unit, Document &doc);
    void get_account(AccountUnitBinanceF& unit, Document &doc);
    void get_listen_key(AccountUnitBinanceF& unit, Document &doc);
    void put_listen_key(AccountUnitBinanceF& unit, Document &doc);
    ...
    SendOrderFilter getSendOrderFilter(AccountUnitBinanceF& unit, const char *symbol);

    bool shouldRetry(int http_status_code, std::string errorMsg, std::string text);
    bool order_count_over_limit(AccountUnitBinanceF& unit);

   

    void handle_request_weight(AccountUnitBinanceF& unit,std::string interface,RequestWeightType type);
    void meet_429(AccountUnitBinanceF& unit);
    bool isHandling(AccountUnitBinanceF& unit);
    
  

    AccountUnitBinanceF& findAccountUnitByWebsocketConn(struct lws * websocketConn);
    void onOrder(AccountUnitBinanceF& unit, Document& json);
    void wsloop();
...
private:
  ...
  //  uint64_t last_test_timestamp = 0;
    ...
    std::map<std::string,AccountUnitBinanceF*> mapInsertOrders;
    AccountUnitBinanceF& get_current_account();
...
#endif //PROJECT_TDEngineBinanceF_H