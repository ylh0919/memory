...
private:
    ...
    void get_open_order(AccountUnitKraken& unit,  Document& json);
    ...
    int64_t getMSTime();
    void handle_order_insert(AccountUnitKraken& unit,const LFInputOrderField data,int requestId,const std::string& ticker);
    void handle_order_action(AccountUnitKraken& unit,const LFOrderActionField data, int requestId,const std::string& ticker);
public:
...
private:
    ...
    ThreadPtr limit_thread;

    uint64_t last_rest_get_ts = 0;
    uint64_t rest_get_interval_ms = 2000;
    uint64_t last_limit_get_ts = 0;
    int limit_count = 15;

    std::mutex* mutex_order_and_trade = nullptr;
    std::mutex* mutex_response_order_status = nullptr;
    std::mutex* mutex_orderaction_waiting_response = nullptr;
    std::mutex* m_mutexlocal = nullptr;
