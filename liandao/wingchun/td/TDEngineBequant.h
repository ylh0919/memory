...
#ifndef PROJECT_TDENGINEBEQUANT_H
#define PROJECT_TDENGINEBEQUANT_H
 ...
 * CTP trade engine
 */
        class TDEngineBequant: public ITDEngine
        {
        public:
            ...
            virtual string name() const { return "TDEngineBequant"; };
        ...
        private:
            ...
            vector<AccountUnitBequant> account_units;
            ...
            //void handlerResponseOrderStatus(AccountUnitBequant& unit, std::vector<PendingOrderStatus>::iterator orderStatusIterator, ResponsedOrderStatus& responsedOrderStatus);
            void addResponsedOrderStatusNoOrderRef(ResponsedOrderStatus &responsedOrderStatus, Document& json);
            void getPriceIncrement(AccountUnitBequant& unit);
            void dealPriceVolume(AccountUnitBequant& unit,const std::string& symbol,int64_t nPrice,int64_t nVolume,double& dDealPrice,double& dDealVome,bool Isbuy);
        ...
        private:
            void get_account(AccountUnitBequant& unit, Document& json, std::string brokerid);
            void send_order(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price, int Id, bool isPostOnly);

            void cancel_all_orders();
            void cancel_order(std::string orderId);
            
            void req_withdraw_currency(const LFWithdrawField* data, int account_index, int requestId);
            void withdraw_currency(string currency, string address, string amount, Document& json, AccountUnitBequant& unit);
            void req_inner_transfer(const LFTransferField* data, int account_index, int requestId);
            void inner_transfer(string currency, string amount, string type, Document& json, AccountUnitBequant& unit);
            void req_transfer_history(const LFTransferHistoryField* data, int account_index, int requestId, bool isWithdraw);
            void get_transfer_history(AccountUnitBequant& unit, Document &json, string currency);
            ...
            std::string createInsertOrderString(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price, int Id, bool isPostOnly);
        ...
        private:     
            std::string makeSubscribeChannelString(AccountUnitBequant& unit);
            std::string makeSubscribetradeString(AccountUnitBequant& unit);
            std::string sign(const AccountUnitBequant& unit,const std::string& method,const std::string& timestamp,const std::string& endpoint);
...
#endif //PROJECT_TDENGINEBEQUANT_H
