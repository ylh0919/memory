 ...
 void TDEngineEmx::onOrderChange(Document& msg)
{
    if(msg.HasMember("type") && msg["type"].GetString() == std::string("update") && msg.HasMember("action") && msg.HasMember("data"))
    {
        ...
        if(strAction == "order-received" && data.HasMember("order_id") && data.HasMember("client_id"))
        {
            ...
            {
	            strGatewayTime = data["timestamp"].GetString();
            }
            ...
            {
                strncpy(it->second.data.BusinessUnit,strOrderId.c_str(),64);
	            strncpy(it->second.data.GatewayTime, strGatewayTime.c_str(), 64);
                m_mapOrder.insert(std::make_pair(strOrderId,it->second.data));
                m_mapNewOrder.insert(std::make_pair(strOrderId,it->second));
        ...
        else if(strAction == "accepted" )
        {
            ...
            if(it != m_mapOrder.end())
            {
	            ...
	            strncpy(it->second.GatewayTime, strGatewayTime.c_str(), 64);
                localOrderRefRemoteOrderId.insert(std::make_pair(it->second.OrderRef,strOrderId));
                on_rtn_order(&(it->second));
                it->second.OrderStatus = LF_CHAR_Unknown ;
            }
            
            auto it2 = m_mapNewOrder.find(strOrderId);
            if(it2 != m_mapNewOrder.end())
            {
        ...
        else if(strAction == "rejected")
        {
            ...
            auto it3 = m_mapNewOrder.find(strOrderId) ;
            if(it3 != m_mapNewOrder.end())
            {
                m_mapNewOrder.erase(it3);
            }
        }
...
void TDEngineEmx::on_lws_connection_error(struct lws* conn)
{
    ...
    if(lws_login(timeout_nsec))
    {
        KF_LOG_ERROR(logger,"zafyes");
        Document json;
        for(auto& unit:account_units)
        {
            cancel_all_orders();
            int64_t currentNano = getMSTime();
            int64_t mintime = currentNano ;
            int64_t rflag = 0 ;
            int64_t mflag = 0 ;

            std::map<std::string, OrderActionSentTime>::iterator itr;
            std::unique_lock<std::mutex> lck(*mutex_orderaction_waiting_response);
            for(itr = remoteOrderIdOrderActionSentTime.begin(); itr != remoteOrderIdOrderActionSentTime.end(); itr++)
            {
                if(currentNano - itr->second.sentNameTime > orderaction_max_waiting_seconds)
                {
            ...
            std::unique_lock<std::mutex> lck1(*m_mutexOrder);
            std::map<std::string,OrderInsert>::iterator it;
            for(it = m_mapNewOrder.begin() ; it != m_mapNewOrder.end() ; it++)
            {
                if(currentNano - it->second.sentNameTime > orderaction_max_waiting_seconds)
                {
                    if(mintime > it->second.sentNameTime)
                    {
                        mintime = it->second.sentNameTime ;
                    }
                    mflag++ ;
                }
            }
            ...
            get_order(unit,mintime,rflag,mflag);
            ...
            for(itr = remoteOrderIdOrderActionSentTime.begin(); itr != remoteOrderIdOrderActionSentTime.end();)
            {
                if(currentNano - itr->second.sentNameTime > orderaction_max_waiting_seconds)
                {
                    auto it2 = m_mapOrderAction.find(itr->first);
                    if(it2 != m_mapOrderAction.end())
                    {
                        on_rsp_order_action(&(it2->second),it2->second.RequestID,101,strError.c_str());
                        m_mapOrderAction.erase(it2);
                    }
            ...
            for(it = m_mapNewOrder.begin() ; it != m_mapNewOrder.end() ; it++)
            {
                if(currentNano - it->second.sentNameTime > orderaction_max_waiting_seconds)
                {
                    auto it2 = m_mapInputOrder.find(it->first);
                    if(it2 != m_mapInputOrder.end())
                    {
                        on_rsp_order_insert(&(it2->second),it->second.data.RequestID,100,strError.c_str());
                        m_mapInputOrder.erase(it2);
                    }
                    m_mapNewOrder.erase(it);
                }
        ...
        m_mapNewOrder.clear();
        m_mapInputOrder.clear();
        m_mapOrderAction.clear();
...
void TDEngineEmx::connect(long timeout_nsec)
{
   ...
   { 
        ...
        //KF_LOG_INFO(logger, "EMX-ACCESS-SIG = " << strSignatrue << ", EMX-ACCESS-TIMESTAMP = " << strTimestamp << ", EMX-API-KEY = " << unit.api_key);
...
bool TDEngineEmx::lws_login(long timeout_nsec)
{
    KF_LOG_INFO(logger, "TDEngineEmx::lws_login:");
	...
	KF_LOG_INFO(logger, "TDEngineEmx::lws_login: context created.");


	if (context == NULL) {
		KF_LOG_ERROR(logger, "TDEngineEmx::lws_login: context is NULL. return");
		return false;
	}
    ...
    KF_LOG_INFO(logger, "TDEngineEmx::lws_login: address = " << clientConnectInfo.address << ",path = " << clientConnectInfo.path);

	wsi = lws_client_connect_via_info(&clientConnectInfo);
	if (wsi == NULL) {
		KF_LOG_ERROR(logger, "TDEngineEmx::lws_login: wsi create error.");
		return false;
	}
	KF_LOG_INFO(logger, "TDEngineEmx::lws_login: wsi create success.");
    is_connecting = true;
    m_conn = wsi;
    return true;
...
void TDEngineEmx::req_order_insert(const LFInputOrderField* data, int account_index, int requestId, long rcv_time)
{
    ...
    }
    {
        ...
        OrderInsert order;
        memset(&order.data, 0, sizeof(LFRtnOrderField));
        order.data.OrderStatus = LF_CHAR_Unknown;
        order.data.VolumeTotalOriginal = std::round(fixedVolume*scale_offset);
        order.data.VolumeTotal = order.data.VolumeTotalOriginal;
        strncpy(order.data.OrderRef, data->OrderRef, 21);
        strncpy(order.data.InstrumentID, data->InstrumentID, 31);
        order.data.RequestID = requestId;
        strcpy(order.data.ExchangeID, "Emx");
        strncpy(order.data.UserID, unit.api_key.c_str(), 16);
        order.data.LimitPrice = std::round(fixedPrice*scale_offset);
...
void TDEngineEmx::addRemoteOrderIdOrderActionSentTime(const LFOrderActionField* data, int requestId, const std::string& remoteOrderId)
{
    ...
    newOrderActionSent.sentNameTime = getMSTime();
...
std::string TDEngineEmx::Timesplit(long long timestamp)
{
    ...
    char timeStr[50]{};
    sprintf(timeStr, "%04d-%02d-%02dT%02d:%02d:%02d.%03dZ", now->tm_year + 1900, now->tm_mon + 1, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec,ms);
    return std::string(timeStr);
}

void TDEngineEmx::get_order(AccountUnitEmx& unit , int64_t mintime , int64_t rflag , int64_t mflag)
{
    std::string timeStr;
    timeStr = Timesplit(mintime) ;
    
    int64_t flag = 0;
    while((rflag != 0 || mflag != 0) && flag < 4)
    {
        std::string requestPath = "/v1/orders?" ;
        if(flag == 1)
        {
            requestPath += "status=rejected&";
        }
        if(flag == 2)
        {
            requestPath += "status=canceled&";
        }
        if(flag == 3)
        {
            requestPath += "status=done&";
        }
        flag++;
        requestPath += "after=";
        int size = 200 ;
        while(size == 200)
        {
            ...
            Document json;
            json.Parse(response.text.c_str());
            if(!json.HasParseError() && json.IsObject() &&json.HasMember("orders"))
            {
                auto& data = json["orders"];
                if(data.IsArray())
                {
                    size = data.Size();
                    ...
                    {
                        auto& item = data[i];
                        if(item.HasMember("order_id") && item["order_id"].IsString() && item.HasMember("status") && item["status"].IsString())
                        {
                            std::string strOrderId = item["order_id"].GetString();
                            std::string strClientId = item["client_id"].GetString();
                            std::string Status = item["status"].GetString();
                            std::string strGatewayTime = item["epoch_timestamp"].GetString();
                            timeStr = strGatewayTime ;
                            if(Status == "accepted" )
                            {
                                std::lock_guard<std::mutex> lck(*m_mutexOrder);
                                auto it = m_mapNewOrder.find(strClientId);
                                if(it != m_mapNewOrder.end())
                                {
                                    ...
                                    it->second.data.OrderStatus = LF_CHAR_NotTouched;
                                    localOrderRefRemoteOrderId.insert(std::make_pair(it->second.data.OrderRef,strOrderId));
                                ...
                                else 
                                {
                                    ...
                                    if(it1 != m_mapOrder.end() && it1->second.OrderStatus == LF_CHAR_Unknown)
                                    {
                                        strncpy(it1->second.BusinessUnit,strOrderId.c_str(),64);
                                        strncpy(it1->second.GatewayTime, strGatewayTime.c_str(), 64);
                                        it1->second.OrderStatus = LF_CHAR_NotTouched;
                                        localOrderRefRemoteOrderId.insert(std::make_pair(it1->second.OrderRef,strOrderId));
                            ...
                            {
                                std::string strError = "order was rejected";
                                std::lock_guard<std::mutex> lck(*m_mutexOrder);
                                auto it = m_mapNewOrder.find(strClientId);
                                if(it != m_mapNewOrder.end())
                                {
                                ...
                                else
                                {
                                    ...
                                    if(it1 != m_mapOrder.end())
                                    {
                                        ...
                                        if(it2 != m_mapInputOrder.end())
                                        {
                                            on_rsp_order_insert(&(it2->second),it1->second.RequestID,100,strError.c_str());
                                            m_mapInputOrder.erase(it2);
                                        }
                                        auto it3 = m_mapNewOrder.find(strOrderId);
                                        if(it3 != m_mapNewOrder.end())
                            ...
                            else if(Status == "canceled")
                            {
                                ...
                                { 
                                    auto it = m_mapOrder.find(strOrderId);
                                    if(it != m_mapOrder.end() && it->second.OrderStatus != LF_CHAR_Canceled)
                                    {
                                        if(it->second.OrderStatus == LF_CHAR_Unknown)
                                        {
                                            it->second.OrderStatus = LF_CHAR_NotTouched;
                                            strncpy(it->second.GatewayTime, strGatewayTime.c_str(), 64);
                                            on_rtn_order(&(it->second));
                            ...
                            else if(Status == "cancel-rejected")
                            {
                                std::string strError = "order was rejected";
                                std::lock_guard<std::mutex> lck(*m_mutexOrder);
                                auto it = m_mapOrder.find(strOrderId);
                            ...
                            else if(Status == "filled")
                            {
                                ...
                                else
                                {
                                    ...
                                    if(it1 != m_mapOrder.end() && it1->second.OrderStatus == LF_CHAR_Unknown)
                                    {
                                        strncpy(it1->second.BusinessUnit,strOrderId.c_str(),64);
                                        strncpy(it1->second.GatewayTime, strGatewayTime.c_str(), 64);
                                        m_mapOrder.insert(std::make_pair(strOrderId,it1->second));

                                        it1->second.OrderStatus = LF_CHAR_NotTouched;
                                        on_rtn_order(&(it1->second));
                                    }
                                }
                                std::string strSizeFilled = item["size_filled"].GetString();
                                std::string strSizeFilledDelta = strSizeFilled;
                                if(item.HasMember("size_filled_delta"))
                                    strSizeFilledDelta = item["size_filled_delta"].GetString();
                                std::string strFillPrice = item["price"].GetString();
                                if(item.HasMember("fill_price"))
                                    strFillPrice = item["fill_price"].GetString();
                                auto it1 = m_mapOrder.find(strOrderId);
                                if(it1 != m_mapOrder.end())
                                {
                                    it1->second.OrderStatus = LF_CHAR_AllTraded;
                                    it1->second.VolumeTraded = std::round(std::stod(strSizeFilled)*scale_offset);
                                    it1->second.VolumeTotal = it1->second.VolumeTotalOriginal - it1->second.VolumeTraded;
                                    strncpy(it1->second.GatewayTime, strGatewayTime.c_str(), 64);
                                    on_rtn_order(&(it1->second));
                                    raw_writer->write_frame(&(it1->second), sizeof(LFRtnOrderField),
                                                            source_id, MSG_TYPE_LF_RTN_ORDER_EMX, 1, -1);

                                    LFRtnTradeField rtn_trade;
                                    memset(&rtn_trade, 0, sizeof(LFRtnTradeField));
                                    strcpy(rtn_trade.ExchangeID,it1->second.ExchangeID);
                                    strncpy(rtn_trade.UserID, it1->second.UserID,sizeof(rtn_trade.UserID));
                                    strncpy(rtn_trade.InstrumentID, it1->second.InstrumentID, sizeof(rtn_trade.InstrumentID));
                                    strncpy(rtn_trade.OrderRef, it1->second.OrderRef, sizeof(rtn_trade.OrderRef));
                                    rtn_trade.Direction = it1->second.Direction;
                                    strncpy(rtn_trade.OrderSysID,strOrderId.c_str(),sizeof(rtn_trade.OrderSysID));
                                    rtn_trade.Volume = std::round(std::stod(strSizeFilledDelta)*scale_offset);
                                    rtn_trade.Price = std::round(std::stod(strFillPrice)*scale_offset);
                                    on_rtn_trade(&rtn_trade);
...
void TDEngineEmx::loopOrderActionNoResponseTimeOut()
{
    ...
    while(isRunning)
    {
        for(auto& unit:account_units)
        {
            orderActionNoResponseTimeOut(unit);
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    }
}

void TDEngineEmx::orderActionNoResponseTimeOut(AccountUnitEmx& unit)
{
    int errorId = 100;
    ...
    for(itr = remoteOrderIdOrderActionSentTime.begin(); itr != remoteOrderIdOrderActionSentTime.end();)
    {
        if(currentNano - itr->second.sentNameTime > orderaction_max_waiting_seconds)
        {
            ...
            if(it2 != m_mapOrderAction.end())
            {
                on_rsp_order_action(&(it2->second),it2->second.RequestID,101,errorMsg.c_str());
                m_mapOrderAction.erase(it2);
            }
            itr = remoteOrderIdOrderActionSentTime.erase(itr);
        } 
 ...
 */
std::string TDEngineEmx::createInsertOrderString(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price,bool is_post_only)
{
...
void TDEngineEmx::send_order(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price,bool is_post_only)
{
