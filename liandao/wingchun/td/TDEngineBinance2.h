...
#ifndef PROJECT_TDENGINEBINANCE2_H
#define PROJECT_TDENGINEBINANCE2_H
...
struct AccountUnitBinance2
{
    ...
    AccountUnitBinance2(const AccountUnitBinance2& source);
    AccountUnitBinance2();
    ~AccountUnitBinance2();
};
...
class TDEngineBinance2: public ITDEngine
{
public:
    ...
    virtual string name() const { return "TDEngineBinance2"; };
    ...
    bool lws_login(AccountUnitBinance2& unit, long timeout_nsec);
...
private:
    ...
    vector<AccountUnitBinance2> account_units;
    ...
    cpr::Response inner_transfer(string asset, string amount, string type, Document& json, AccountUnitBinance2& unit);
    cpr::Response sub_inner_transfer(string fromEmail, string toEmail, string asset, string amount, Document& json, AccountUnitBinance2& unit);
    void withdrawl_currency(string asset, string address, string addresstag, string amount, Document& json, AccountUnitBinance2& unit);
    bool loadExchangeOrderFilters(AccountUnitBinance2& unit, Document &doc);
    inline void onRspNewOrderRESULT(const LFInputOrderField* data, AccountUnitBinance2& unit, Document& result, int requestId);
    inline void onRspNewOrderFULL(const LFInputOrderField* data, AccountUnitBinance2& unit, Document& result, int requestId);
    void onRtnNewOrder(const LFInputOrderField* data, AccountUnitBinance2& unit, int requestId,string remoteOrderId,int64_t fixedVolume,int64_t fixedPrice);
    void get_deposit_history(AccountUnitBinance2& unit, Document &json, string asset , int status , string startTime , string endTime );
    void get_withdraw_history(AccountUnitBinance2& unit, Document &json, string asset , int status , string startTime , string endTime );
    ...
    int64_t getTimeDiffOfExchange(AccountUnitBinance2& unit);
    std::string parseJsonToString(Document &d);
private:
    int HTTP_RESPONSE_OK = 200;
    void send_order(AccountUnitBinance2& unit, const char *symbol,
                    const char *side,
                    const char *type,
    ...
    void send_margin_order(AccountUnitBinance2& unit, const char *symbol,
                const char *side,
                const char *type,
                const char *timeInForce,
                double quantity,
    ...
    void get_margin_open_orders(AccountUnitBinance2& unit, const char *symbol, Document &json);
    void cancel_margin_order(AccountUnitBinance2& unit, const char *symbol,
                  long orderId, const char *origClientOrderId, const char *newClientOrderId, Document &json);
    void get_order(AccountUnitBinance2& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json);
    void cancel_order(AccountUnitBinance2& unit, const char *symbol,
                      long orderId, const char *origClientOrderId, const char *newClientOrderId, Document &doc);
    void get_my_trades(AccountUnitBinance2& unit, const char *symbol, int limit, int64_t fromId, Document &doc);
    void get_open_orders(AccountUnitBinance2& unit, const char *symbol, Document &doc);
    void get_exchange_infos(AccountUnitBinance2& unit, Document &doc);
    void get_exchange_time(AccountUnitBinance2& unit, Document &doc);
    void get_account(AccountUnitBinance2& unit, Document &doc);
    void get_margin_account(AccountUnitBinance2& unit, Document &json);
    void get_sub_account(string email, AccountUnitBinance2& unit, Document &json);
    void get_listen_key(AccountUnitBinance2& unit, Document &doc);
    void put_listen_key(AccountUnitBinance2& unit, Document &doc);
    void get_margin_order(AccountUnitBinance2& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json);
    void get_margin_listen_key(AccountUnitBinance2& unit, Document &json);
    void put_margin_listen_key(AccountUnitBinance2& unit, Document &json);
    ...
    SendOrderFilter getSendOrderFilter(AccountUnitBinance2& unit, const char *symbol);

    bool shouldRetry(int http_status_code, std::string errorMsg, std::string text);
    bool order_count_over_limit(AccountUnitBinance2& unit);

   

    void handle_request_weight(AccountUnitBinance2& unit,RequestWeightType type);
    void meet_429(AccountUnitBinance2& unit);
    bool isHandling(AccountUnitBinance2& unit);
    
  

    AccountUnitBinance2& findAccountUnitByWebsocketConn(struct lws * websocketConn);
    void onOrder(AccountUnitBinance2& unit, Document& json);
    void wsloop();
...
private:
  ...
  //  uint64_t last_test_timestamp = 0;
    ...
    std::map<std::string,AccountUnitBinance2*> mapInsertOrders;
    AccountUnitBinance2& get_current_account();
    AccountUnitBinance2& get_account_from_orderref(std::string& ref);
};

WC_NAMESPACE_END

#endif //PROJECT_TDENGINEBINANCE2_H
