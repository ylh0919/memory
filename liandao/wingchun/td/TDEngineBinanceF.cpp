...
static TDEngineBinanceF* global_td = nullptr;
...
AccountUnitBinanceF::AccountUnitBinanceF()
{
...
AccountUnitBinanceF::AccountUnitBinanceF(const AccountUnitBinanceF& source)
{
...
TDEngineBinanceF::TDEngineBinanceF(): ITDEngine(SOURCE_BINANCEF)
{
    logger = yijinjing::KfLog::getLogger("TradeEngine.BinanceF");
    KF_LOG_INFO(logger, "[ATTENTION] default to confirm settlement and no authentication!");
}

TDEngineBinanceF::~TDEngineBinanceF()
{
    if(m_ThreadPoolPtr!=nullptr) delete m_ThreadPoolPtr;
}

void TDEngineBinanceF::init()
{
    ITDEngine::init();
...
void TDEngineBinanceF::pre_load(const json& j_config)
{
    KF_LOG_INFO(logger, "[pre_load]");
}

void TDEngineBinanceF::resize_accounts(int account_num)
{
    
}

TradeAccount TDEngineBinanceF::load_account(int idx, const json& j_config)
{
    ...
    KF_LOG_INFO(logger, "[load_account] (restBaseUrl)" << restBaseUrl);
    KF_LOG_INFO(logger, "[load_account] (wsBaseUrl)"   << wsBaseUrl);
    ...
    //testnet dstream.binancefuture.com
    if ((restBaseUrl.find("dapi.binance.com") != -1 && wsBaseUrl.find("dstream.binance.com") != -1) ||
        (restBaseUrl.find("testnet.binancefuture.com") != -1 && wsBaseUrl.find("dstream.binancefuture.com") != -1)) {
    ...
    { 
        for (auto& j_account: iter.value())
        {
            ...
            if(unit.coinPairWhiteList.Size() == 0) {
                //display usage:
                KF_LOG_ERROR(logger, "TDEngineBinanceF::load_account: subscribeCoinBaseQuote is empty. please add whiteLists in kungfu.json like this :");
...
void TDEngineBinanceF::genUniqueKey ()
{
...
std::string TDEngineBinanceF::genClientid (const std::string &orderRef)
{
    return m_uniqueKey + orderRef;
}

void TDEngineBinanceF::connect(long timeout_nsec)
{
    KF_LOG_INFO(logger, "[connect]");
    //std::unique_lock<std::mutex> lck(mutex_account);
    for (size_t idx = 0; idx < account_units.size(); idx++)
    {
        AccountUnitBinanceF& unit = account_units[idx];
...
bool TDEngineBinanceF::loadExchangeOrderFilters(AccountUnitBinanceF& unit, Document &doc)
{
...
void TDEngineBinanceF::debug_print(std::map<std::string, SendOrderFilter> &sendOrderFilters)
{
...
SendOrderFilter TDEngineBinanceF::getSendOrderFilter(AccountUnitBinanceF& unit, const char *symbol)
{
...
void TDEngineBinanceF::login(long timeout_nsec)
{
    KF_LOG_INFO(logger, "[login]");
    connect(timeout_nsec);
}

void TDEngineBinanceF::logout()
{
    KF_LOG_INFO(logger, "[logout]");
}

void TDEngineBinanceF::release_api()
{
    KF_LOG_INFO(logger, "[release_api]");
}

bool TDEngineBinanceF::is_logged_in() const
{
    KF_LOG_INFO(logger, "[is_logged_in] account_units.size()=" << account_units.size());
    //std::unique_lock<std::mutex> lck(mutex_account);
    for (auto& unit: account_units)
    {
        if (!unit.logged_in) {
            KF_LOG_INFO(logger, "[is_logged_in] return false");
            return false;
        }
            
    }
  //  lck.unlock();
    KF_LOG_INFO(logger, "[is_logged_in] return true");
    return true;
}

bool TDEngineBinanceF::is_connected() const
{
...
std::string TDEngineBinanceF::GetSide(const LfDirectionType& input) {
 ...
 LfDirectionType TDEngineBinanceF::GetDirection(std::string input) {
    if ("BUY" == input) {
        return LF_CHAR_Buy;
    } else if ("SELL" == input) {
        return LF_CHAR_Sell;
...
std::string TDEngineBinanceF::GetType(const LfOrderPriceTypeType& input) {
...
LfOrderPriceTypeType TDEngineBinanceF::GetPriceType(std::string input) {
    if ("LIMIT" == input) {
...
std::string TDEngineBinanceF::GetTimeInForce(const LfTimeConditionType& input) {
...
LfTimeConditionType TDEngineBinanceF::GetTimeCondition(std::string input) {
    if ("IOC" == input) {
      return LF_CHAR_IOC;
    } else if ("GTC" == input) {
      return LF_CHAR_GTC;
...
LfOrderStatusType TDEngineBinanceF::GetOrderStatus(std::string input) {
    if ("NEW" == input) {
...
AccountUnitBinanceF& TDEngineBinanceF::get_current_account()
{
    current_account_idx++;
    current_account_idx %= account_units.size();
    return account_units[current_account_idx];
}
AccountUnitBinanceF& get_account_from_orderref(std::string& ref)
{
 ...
 * req functions
 */
void TDEngineBinanceF::req_investor_position(const LFQryPositionField* data, int account_index, int requestId)
{
    KF_LOG_INFO(logger, "[req_investor_position]");
    //std::unique_lock<std::mutex> lck(mutex_account);
    AccountUnitBinanceF& unit = account_units[0];
    //lck.unlock();
    ...
    send_writer->write_frame(data, sizeof(LFQryPositionField), source_id, MSG_TYPE_LF_QRY_POS_BINANCEF, 1, requestId);
    ...
    if(errorId != 0)
    {
        raw_writer->write_error_frame(&pos, sizeof(LFRspPositionField), source_id, MSG_TYPE_LF_RSP_POS_BINANCEF, 1, requestId, errorId, errorMsg.c_str());
    }
}

void TDEngineBinanceF::req_qry_account(const LFQryAccountField *data, int account_index, int requestId)
{
    KF_LOG_INFO(logger, "[req_qry_account]");
}


int64_t TDEngineBinanceF::fixPriceTickSize(int keepPrecision, int64_t price, bool isBuy)
{
...
int64_t TDEngineBinanceF::fixVolumeStepSize(int keepPrecision, int64_t volume, bool isBuy)
{
    //the 8 is come from 1e8.
...
void TDEngineBinanceF::send_order_thread(SendOrderParam param, const LFInputOrderField data,std::string account_type,int requestId,int64_t timestamp, double fixedVolume,double fixedPrice)
{
    ...
    raw_writer->write_error_frame(&data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_BINANCEF, 1, requestId, errorId, errorMsg.c_str());
    ...
    if(!d.HasParseError() && d.IsObject() && !d.HasMember("code")&&d.HasMember("orderId"))
    {
        ...
        if(tif == "GTC")
        {
            ...
            {
                KF_LOG_ERROR(logger, "TDEngineBinanceF::send_order_thread can't find correcr unit" );
                return;
            }
        ...
        if(it != param.unit->newordersMap.end())
        {
            ...
            //fix MSG_TYPE_LF_RTN_ORDER_BINANCE to MSG_TYPE_LF_RTN_ORDER_BINANCEF
                            source_id, MSG_TYPE_LF_RTN_ORDER_BINANCEF,
...
void TDEngineBinanceF::req_order_insert(const LFInputOrderField* data, int account_index, int requestId, long rcv_time)
{
    //yijinjing::FramePtr frame;
    AccountUnitBinanceF& unit = get_current_account();
    KF_LOG_DEBUG(logger, "[req_order_insert]" << " (rid)" << requestId
                                              << " (APIKey)" << unit.api_key
                                              << " (Tid)" << data->InstrumentID
                                              << " (OrderRef)" << data->OrderRef
                                              << " (TimeCondition)" << data->TimeCondition);
    send_writer->write_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_BINANCEF, 1/*ISLAST*/, requestId);
   ...
   time_t curTime = time(0);
        ...
        if ((it_rate->second.order_count+1) >= UFR_order_lower_limit)
        {
            ...
            if (UFR >= UFR_limit)
            {
                ...
                KF_LOG_ERROR(logger, "[req_order_insert]: UFR above limit: (rid)" << requestId << " (errorId)" <<
                        errorId << " (errorMsg) " << errorMsg);
                on_rsp_order_insert(data, requestId, errorId, errorMsg.c_str());//write in _ info
                raw_writer->write_error_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_BINANCEF, 1, requestId, errorId, errorMsg.c_str());//write in _ info
                return;
            }
        }      
    }
    ...
    {
        m_ThreadPoolPtr->commit(std::bind(&TDEngineBinanceF::send_order_thread,this,param,*data,account_type,requestId,timestamp,fixedVolume,fixedPrice));
    }
...
void TDEngineBinanceF::onRtnNewOrder(const LFInputOrderField* data, AccountUnitBinanceF& unit, int requestId,string remoteOrderId,int64_t fixedVolume,int64_t fixedPrice)
{
...
void TDEngineBinanceF::action_order_thread(AccountUnitBinanceF* unit,  string ticker,const LFOrderActionField data, std::string account_type,int requestId  )
{
    ...
    //     lck1.unlock();
        ...
        { 
            ...
            //on_rsp_order_action(data, requestId, errorId, errorMsg.c_str());
            KF_LOG_ERROR_FMT(logger, "TDEngineBinanceF::onOrder,no order match(%s),cancleFailed",remoteOrderId.c_str());
            //on_rsp_order_action(&data, requestId, errorId, errorMsg.c_str());
            return;
        }
        ...
        KF_LOG_INFO(logger, "TDEngineBinanceF::req_order_action ,rtn_order");
        on_rtn_order(&rtn_order);
        //fix MSG_TYPE_LF_RTN_ORDER_BINANCE to MSG_TYPE_LF_RTN_ORDER_BINANCEF
        raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),source_id, MSG_TYPE_LF_RTN_ORDER_BINANCEF,1, (rtn_order.RequestID > 0) ? rtn_order.RequestID : -1);
    ...
    //fix MSG_TYPE_LF_ORDER_ACTION_BINANCE to MSG_TYPE_LF_ORDER_ACTION_BINANCEF
    raw_writer->write_error_frame(&data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCEF, 1, requestId, errorId, errorMsg.c_str());
}

void TDEngineBinanceF::batch_cancel_order_thread(AccountUnitBinanceF* unit, string ticker, const LFBatchCancelOrderField* data, std::string account_type, int requestId)
{
    ...
    if (!docs.HasParseError() && docs.IsArray()) {
        for (int i = 0; i < docs.Size(); i++) {
            ...
            else
            {
                ...
                if (it == unit->ordersMap.end())
                {
                    KF_LOG_DEBUG_FMT(logger, "TDEngineBinanceF::onOrder,cannot find remoteOrderId(%s) in ordersMap, check clientOrderId", remoteOrderId.c_str());
                    std::string clientOrderId = d["clientOrderId"].GetString();
                    if (clientOrderId == genClientid(data->InfoList[i].OrderRef))
                        KF_LOG_DEBUG_FMT(logger, "TDEngineBinanceF::onOrder,clientOrderId(%s) == genClientid(data->InfoList[i].OrderRef)", clientOrderId.c_str());
                    ...
                    rtn_order.VolumeTraded += volumeTraded;
                    rtn_order.VolumeTotal = rtn_order.VolumeTotalOriginal - rtn_order.VolumeTraded;
                    KF_LOG_INFO(logger, "TDEngineBinanceF::batch_cancel_order_thread ,rtn_order");
                    on_rtn_order(&rtn_order);
                    raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField), source_id, MSG_TYPE_LF_RTN_ORDER_BINANCEF, 1, (rtn_order.RequestID > 0) ? rtn_order.RequestID : -1);
            ...
            raw_writer->write_error_frame(&tmp, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCEF, 1, requestId, errorId, errorMsg.c_str());
            //这3种情况对于 该组所有订单都有影响
...
void TDEngineBinanceF::req_order_action(const LFOrderActionField* data, int account_index, int requestId, long rcv_time)
{
    ...
    AccountUnitBinanceF& unit = *(it->second);
    lck.unlock();
    ...
    send_writer->write_frame(data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCEF, 1, requestId);
    ...
    {
        m_ThreadPoolPtr->commit(std::bind(&TDEngineBinanceF::action_order_thread,this,&unit,ticker,*data,account_type,requestId));
    }
}

LFOrderActionField TDEngineBinanceF::parseFrom(const LFBatchCancelOrderField* data, int index) {
...
void TDEngineBinanceF::init(const LFBatchCancelOrderField* data, LFBatchCancelOrderField* res)
{
...
void TDEngineBinanceF::req_batch_cancel_orders(const LFBatchCancelOrderField* data, int account_index, int requestId, long rcv_time) {
    int errorId = 0;
    std::string errorMsg = "";
    LFOrderActionField tmp = parseFrom(data, 0);

    std::map<LFBatchCancelOrderField*, AccountUnitBinanceF*> data_unit;
    ...
    for (int i = 0; i < data->SizeOfList; i++) {
        ...
        if (data_unit.size() != 0) {
            ...
            std::map<LFBatchCancelOrderField*, AccountUnitBinanceF*>::iterator itr;
    ...
    for (auto itr : data_unit) {
        AccountUnitBinanceF* unit = itr.second;
        send_writer->write_frame(itr.first, sizeof(LFBatchCancelOrderField), source_id, MSG_TYPE_LF_BATCH_CANCEL_ORDER_BINANCEF, 1, requestId);
        ...
        {
            m_ThreadPoolPtr->commit(std::bind(&TDEngineBinanceF::batch_cancel_order_thread, this, unit, ticker, itr.first, account_type, requestId));
        }
    }
...
void TDEngineBinanceF::req_withdraw_currency(const LFWithdrawField* data, int account_index, int requestId){
   AccountUnitBinanceF& unit = account_units[account_index];
    KF_LOG_DEBUG(logger, "[req_withdraw_currency]" << " (rid) " << requestId
                                              ...
                                              << " (Tag) " << data->Tag);
    send_writer->write_frame(data, sizeof(LFWithdrawField), source_id, MSG_TYPE_LF_WITHDRAW_BINANCEF, 1, requestId);
    int errorId = 0;
    std::string errorMsg = "";
    ...
    if(address == ""){
        ...
        raw_writer->write_error_frame(data, sizeof(LFWithdrawField), source_id, MSG_TYPE_LF_WITHDRAW_BINANCEF, 1, 
...
int64_t TDEngineBinanceF::get_millsecond_interval(string interval) {
    int res;
...
void TDEngineBinanceF::req_get_kline_via_rest(const GetKlineViaRest* data, int account_index, int requestId, long rcv_time)
{
    ...
    //writer->write_frame(data, sizeof(GetKlineViaRest), source_id, MSG_TYPE_LF_GET_KLINE_VIA_REST_BINANCEF, 1/*islast*/, requestId);
    ...
    if (d.IsArray()) {
        ...
        strcpy(bars.ExchangeID, "binancef");
        int j = 0;
...
void TDEngineBinanceF::withdrawl_currency(string asset, string address, string addresstag, string amount, Document& json, AccountUnitBinanceF& unit){
...
void TDEngineBinanceF::set_reader_thread()
{
    ITDEngine::set_reader_thread();

    KF_LOG_INFO(logger, "[set_reader_thread] rest_thread start on AccountUnitBinanceF::loop");
    rest_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinanceF::loop, this)));

    KF_LOG_INFO(logger,"[set_reader_thread] cancle_thread start on AccountUnitBinanceF::caloop");
    cancel_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinanceF::caloop,this)));

   // KF_LOG_INFO(logger,"[set_reader_thread] send_thread start on AccountUnitBinanceF::seloop");
  //  send_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinanceF::seloop,this)));

    // //仿照上面在这里创建新线程，loop是创建的线程里的主函数
    // KF_LOG_INFO(logger,"[set_reader_thread] rest_thread start on AccountUnitBinanceF::testUTC");
    // test_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinanceF::testUTC,this)));
}
void TDEngineBinanceF::loop()
{
...
void TDEngineBinanceF::caloop()
{
    ...
    while(isRunning)
    {
        ...
        for( it = mapCancelOrder.begin(); it != mapCancelOrder.end(); )
        {
            ...
            {
                AccountUnitBinanceF& unit = it->second.unit;
                ...
                else if(json.HasMember("status"))
                {
                    ...
                    if(GetOrderStatus(status) != LF_CHAR_Canceled){
                        ...
                        raw_writer->write_error_frame(&data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCEF, 1, it->second.request_id, errorId, errorMsg.c_str());
                    }
                    else
                    {
                        std::unique_lock<std::mutex> lck1(*unit.mutex_order_and_trade);
...
std::vector<std::string> TDEngineBinanceF::split(std::string str, std::string token)
{
...
void TDEngineBinanceF::send_order(AccountUnitBinanceF& unit, const char *symbol,
                const char *side,
                const char *type,
 ...
 * */
bool TDEngineBinanceF::shouldRetry(int http_status_code, std::string errorMsg, std::string text)
{
...
bool TDEngineBinanceF::order_count_over_limit(AccountUnitBinanceF& unit)
{
...
void TDEngineBinanceF::handle_request_weight(AccountUnitBinanceF& unit,std::string interface,RequestWeightType type)
{
...
void TDEngineBinanceF::meet_429(AccountUnitBinanceF& unit)
{
...
bool TDEngineBinanceF::isHandling(AccountUnitBinanceF& unit)
{
    std::lock_guard<std::mutex> guard_mutex(*unit.mutex_handle_429);
    uint64_t timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
void TDEngineBinanceF::get_order(AccountUnitBinanceF& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json)
{
...
void TDEngineBinanceF::cancel_order(AccountUnitBinanceF& unit, const char *symbol,
...
void TDEngineBinanceF::batch_cancel_orders(AccountUnitBinanceF& unit, const char* symbol, std::vector<long> orderIdList, std::vector<std::string> origClientOrderIdList, Document& json)
{
...
void TDEngineBinanceF::get_my_trades(AccountUnitBinanceF& unit, const char *symbol, int limit, int64_t fromId, Document &json)
{
...
void TDEngineBinanceF::get_open_orders(AccountUnitBinanceF& unit, const char *symbol, Document &json)
{
...
void TDEngineBinanceF::get_exchange_time(AccountUnitBinanceF& unit, Document &json)
{
...
void TDEngineBinanceF::get_exchange_infos(AccountUnitBinanceF& unit, Document &json)
{
...
void TDEngineBinanceF::get_account(AccountUnitBinanceF& unit, Document &json)
{
...
void TDEngineBinanceF::get_listen_key(AccountUnitBinanceF& unit, Document &json)
{
...
void TDEngineBinanceF::put_listen_key(AccountUnitBinanceF& unit, Document &json)
{
...
void TDEngineBinanceF::printResponse(const Document& d)
{
...
void TDEngineBinanceF::getResponse(int http_status_code, std::string responseText, std::string errorMsg, Document& json)
{
    json.Parse(responseText.c_str());
}

int64_t TDEngineBinanceF::getTimestamp()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    return timestamp;
}

std::string TDEngineBinanceF::getTimestampString()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
int64_t TDEngineBinanceF::getTimeDiffOfExchange(AccountUnitBinanceF& unit)
{
...
void TDEngineBinanceF::on_lws_connection_error(struct lws* conn)
{
    KF_LOG_ERROR(logger, "TDEngineBinanceF::on_lws_connection_error.");
    AccountUnitBinanceF& unit = findAccountUnitByWebsocketConn(conn);
...
int TDEngineBinanceF::lws_write_subscribe(struct lws* conn)
{
    return 0;
}

bool TDEngineBinanceF::lws_login(AccountUnitBinanceF& unit, long timeout_nsec) {
    KF_LOG_INFO(logger, "TDEngineBinanceF::lws_login:");
    global_td = this;
    //
    unit.context = NULL;
    int errorId = 0;
    ...
    if (unit.context == NULL) {
        ...
        KF_LOG_INFO(logger, "TDEngineBinanceF::lws_login: context created:"<< unit.api_key);
    }

    if (unit.context == NULL) {
        KF_LOG_ERROR(logger, "TDEngineBinanceF::lws_login: context of" << unit.api_key <<" is NULL. return");
        return false;
    }
    ...
    KF_LOG_INFO(logger, "TDEngineBinanceF::lws_login: Connecting to " <<  ccinfo.host << ":" << ccinfo.port << ":" << ccinfo.path);

    if (unit.websocketConn == NULL) {
        KF_LOG_ERROR(logger, "TDEngineBinanceF::lws_login: wsi create error.");
        return false;
    }
    unit.last_put_time = getTimestamp();
    KF_LOG_INFO(logger, "TDEngineBinanceF::lws_login: wsi create success.");
    unit.is_connecting = true;
    return true;
}


void TDEngineBinanceF::on_lws_data(struct lws* conn, const char* data, size_t len) {// web sock
    AccountUnitBinanceF &unit = findAccountUnitByWebsocketConn(conn);
    KF_LOG_INFO(logger, "TDEngineBinanceF::on_lws_data: " << data);
    Document json;
    json.Parse(data,len);
    if (json.HasParseError() || !json.IsObject()) {
        KF_LOG_ERROR(logger, "TDEngineBinanceF::on_lws_data. parse json error: " << data);    //判断是否有json格式错误    
    }
    else if(json.HasMember("e"))    //判断Event Type
    {
        ...
        {
            KF_LOG_INFO(logger, "TDEngineBinanceF::on_lws_data. Order Update ");    //订单状态更新，处理
            AccountUnitBinanceF& unit= findAccountUnitByWebsocketConn(conn);
            std::lock_guard<std::mutex> lck(*unit.mutex_order_and_trade);
            onOrder(unit,json);
        }
        ...
        {
            KF_LOG_INFO(logger, "TDEngineBinanceF::on_lws_data. Account Update ");   //不处理
        }
...
AccountUnitBinanceF& TDEngineBinanceF::findAccountUnitByWebsocketConn(struct lws * websocketConn)
{
    for (size_t idx = 0; idx < account_units.size(); idx++) {
        AccountUnitBinanceF &unit = account_units[idx];
...
//cys add
std::string TDEngineBinanceF::parseJsonToString(Document &d){
    StringBuffer buffer;
    Writer<StringBuffer> writer(buffer);
    d.Accept(writer);
 ...
 */
void TDEngineBinanceF::onOrder(AccountUnitBinanceF& unit, Document& json) {
    KF_LOG_INFO(logger, "TDEngineBinanceF::onOrder");
    if(json.HasMember("o"))
    {
        ...
        if (record.IsObject()) {
            if (record.HasMember("c")&& record.HasMember("i") && record.HasMember("X")&& record.HasMember("l")&& record.HasMember("L")&& record.HasMember("z")&& record.HasMember("t")) 
            {
                ...
                { 
                    ...
                    if (it == unit.ordersMap.end())
                    {
                        KF_LOG_ERROR(logger, "TDEngineBinanceF::onOrder,no order match:" << clientOrderId);
                        return;
                    }
                }
                else
                ...
                {
                    KF_LOG_INFO(logger, "TDEngineBinanceF::onOrder,status is not changed");
                    return;
                }
                ...
                KF_LOG_INFO(logger, "TDEngineBinanceF::[OnOrder] (request__ID)"<<rtn_order.RequestID<<" (old_status) "<<oldOrderStatus<<"(current_startus)"<<status);
                if(!(oldOrderStatus == status && status == LF_CHAR_Canceled))
                {//与req_order_action函数中返回的 canceled 互补，先收到交易所数据的先处理，后收到的不处理
                    KF_LOG_INFO(logger, "TDEngineBinanceF::onOrder,rtn_order");
                    on_rtn_order(&rtn_order);
                    raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),source_id, MSG_TYPE_LF_RTN_ORDER_BINANCEF,1, (rtn_order.RequestID > 0) ? rtn_order.RequestID : -1);
                }    
                ...
                KF_LOG_INFO(logger, "TDEngineBinanceF::onOrder,rtn_trade");
                if(oldVolumeTraded != rtn_order.VolumeTraded){
                    on_rtn_trade(&rtn_trade);
                    raw_writer->write_frame(&rtn_trade, sizeof(LFRtnTradeField),source_id, MSG_TYPE_LF_RTN_TRADE_BINANCEF, 1, -1);
                }
                //---------------成交总量+1------------------------
...
BOOST_PYTHON_MODULE(libbinanceftd)
{
    using namespace boost::python;
    class_<TDEngineBinanceF, boost::shared_ptr<TDEngineBinanceF> >("Engine")
    .def(init<>())
    .def("init", &TDEngineBinanceF::initialize)
    .def("start", &TDEngineBinanceF::start)
    .def("stop", &TDEngineBinanceF::stop)
    .def("logout", &TDEngineBinanceF::logout)
    .def("wait_for_stop", &TDEngineBinanceF::wait_for_stop);
}
