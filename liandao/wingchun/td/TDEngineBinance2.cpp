...
static TDEngineBinance2* global_td = nullptr;
...
AccountUnitBinance2::AccountUnitBinance2()
{
...
AccountUnitBinance2::AccountUnitBinance2(const AccountUnitBinance2& source)
{
...
TDEngineBinance2::TDEngineBinance2(): ITDEngine(SOURCE_BINANCE2)
{
    logger = yijinjing::KfLog::getLogger("TradeEngine.Binance2");
...
TDEngineBinance2::~TDEngineBinance2()
{
}

void TDEngineBinance2::init()
{
    ITDEngine::init();
...
void TDEngineBinance2::pre_load(const json& j_config)
{
    KF_LOG_INFO(logger, "[pre_load]");
}

void TDEngineBinance2::resize_accounts(int account_num)
{
    
}

TradeAccount TDEngineBinance2::load_account(int idx, const json& j_config)
{
    ...
    { 
        for (auto& j_account: iter.value())
        {
            ...
            if(unit.coinPairWhiteList.Size() == 0) {
                //display usage:
                KF_LOG_ERROR(logger, "TDEngineBinance2::load_account: subscribeCoinBaseQuote is empty. please add whiteLists in kungfu.json like this :");
...
void TDEngineBinance2::connect(long timeout_nsec)
{
    KF_LOG_INFO(logger, "[connect]");
    for (size_t idx = 0; idx < account_units.size(); idx++)
    {
        AccountUnitBinance2& unit = account_units[idx];
...
bool TDEngineBinance2::loadExchangeOrderFilters(AccountUnitBinance2& unit, Document &doc)
{
...
void TDEngineBinance2::debug_print(std::map<std::string, SendOrderFilter> &sendOrderFilters)
{
...
SendOrderFilter TDEngineBinance2::getSendOrderFilter(AccountUnitBinance2& unit, const char *symbol)
{
...
void TDEngineBinance2::login(long timeout_nsec)
{
    KF_LOG_INFO(logger, "[login]");
    connect(timeout_nsec);
}

void TDEngineBinance2::logout()
{
    KF_LOG_INFO(logger, "[logout]");
}

void TDEngineBinance2::release_api()
{
    KF_LOG_INFO(logger, "[release_api]");
}

bool TDEngineBinance2::is_logged_in() const
{
...
bool TDEngineBinance2::is_connected() const
{
...
std::string TDEngineBinance2::GetSide(const LfDirectionType& input) {
 ...
 LfDirectionType TDEngineBinance2::GetDirection(std::string input) {
    if ("BUY" == input) {
        return LF_CHAR_Buy;
    } else if ("SELL" == input) {
        return LF_CHAR_Sell;
...
std::string TDEngineBinance2::GetType(const LfOrderPriceTypeType& input) {
...
LfOrderPriceTypeType TDEngineBinance2::GetPriceType(std::string input) {
    if ("LIMIT" == input) {
...
std::string TDEngineBinance2::GetTimeInForce(const LfTimeConditionType& input) {
...
LfTimeConditionType TDEngineBinance2::GetTimeCondition(std::string input) {
    if ("IOC" == input) {
      return LF_CHAR_IOC;
    } else if ("GTC" == input) {
      return LF_CHAR_GTC;
...
LfOrderStatusType TDEngineBinance2::GetOrderStatus(std::string input) {
    if ("NEW" == input) {
...
AccountUnitBinance2& TDEngineBinance2::get_current_account()
{
    current_account_idx++;
    current_account_idx %= account_units.size();
    return account_units[current_account_idx];
}
AccountUnitBinance2& get_account_from_orderref(std::string& ref)
{
 ...
 * req functions
 */
void TDEngineBinance2::req_investor_position(const LFQryPositionField* data, int account_index, int requestId)
{
  KF_LOG_INFO(logger, "[req_investor_position]");
    AccountUnitBinance2& unit = account_units[0];
    ...
    if( !strcmp(data->BrokerID, "master-main"))
    {
        ...
        raw_writer->write_error_frame(&pos, sizeof(LFRspPositionField), source_id, MSG_TYPE_LF_RSP_POS_BINANCE2, 1, requestId, errorId, errorMsg.c_str());
        return ;
    }
    printResponse(d);
    ...
    send_writer->write_frame(data, sizeof(LFQryPositionField), source_id, MSG_TYPE_LF_QRY_POS_BINANCE2, 1, requestId);
...
void TDEngineBinance2::req_qry_account(const LFQryAccountField *data, int account_index, int requestId)
{
    KF_LOG_INFO(logger, "[req_qry_account]");
}


int64_t TDEngineBinance2::fixPriceTickSize(int keepPrecision, int64_t price, bool isBuy)
{
...
int64_t TDEngineBinance2::fixVolumeStepSize(int keepPrecision, int64_t volume, bool isBuy)
{
    //the 8 is come from 1e8.
...
void TDEngineBinance2::req_order_insert(const LFInputOrderField* data, int account_index, int requestId, long rcv_time)
{
    AccountUnitBinance2& unit = get_current_account();
    KF_LOG_DEBUG(logger, "[req_order_insert]" << " (rid)" << requestId
                                              << " (APIKey)" << unit.api_key
                                              << " (Tid)" << data->InstrumentID
                                              << " (OrderRef)" << data->OrderRef);
    send_writer->write_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_BINANCE2, 1/*ISLAST*/, requestId);
    ...
    if(ticker.length() == 0) {
        ...
        raw_writer->write_error_frame(data, sizeof(LFInputOrderField), source_id, MSG_TYPE_LF_ORDER_BINANCE2, 1, requestId, errorId, errorMsg.c_str());
        return;
    }
    ...
    if( account_type == "margin")
    {
        send_margin_order(unit, ticker.c_str(), GetSide(data->Direction).c_str(), GetType(data->OrderPriceType).c_str(),
        GetTimeInForce(data->TimeCondition).c_str(), fixedVolume*1.0/scale_offset, fixedPrice*1.0/scale_offset, stopPrice, data->OrderRef,
        icebergQty, active, d);
...
void TDEngineBinance2::onRtnNewOrder(const LFInputOrderField* data, AccountUnitBinance2& unit, int requestId,string remoteOrderId,int64_t fixedVolume,int64_t fixedPrice)
{
    ...
    raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),
                            source_id, MSG_TYPE_LF_RTN_ORDER_BINANCE2,
...
void TDEngineBinance2::req_order_action(const LFOrderActionField* data, int account_index, int requestId, long rcv_time)
{
    std::unique_lock<std::mutex> lck(account_mutex);
    int errorId = 0;
    std::string errorMsg = "";
    ...
    if(it == mapInsertOrders.end())
    {
        ...
        raw_writer->write_error_frame(data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCE2, 1, requestId, errorId, errorMsg.c_str());
        return;
    }
    AccountUnitBinance2& unit = *(it->second);
    lck.unlock();
    ...
    send_writer->write_frame(data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCE2, 1, requestId);
    ...
    if( account_type == "margin")
    {
        cancel_margin_order(unit, ticker.c_str(), 0, data->OrderRef, "", d);
...
void TDEngineBinance2::req_withdraw_currency(const LFWithdrawField* data, int account_index, int requestId){
    AccountUnitBinance2& unit = account_units[account_index];
    KF_LOG_DEBUG(logger, "[req_withdraw_currency]" << " (rid) " << requestId
                                              ...
                                              << " (Tag) " << data->Tag);
    send_writer->write_frame(data, sizeof(LFWithdrawField), source_id, MSG_TYPE_LF_WITHDRAW_BINANCE2, 1, requestId);
    int errorId = 0;
    std::string errorMsg = "";
    ...
    if(address == ""){
        ...
        raw_writer->write_error_frame(data, sizeof(LFWithdrawField), source_id, MSG_TYPE_LF_WITHDRAW_BINANCE2, 1, 
...
void TDEngineBinance2::withdrawl_currency(string asset, string address, string addresstag, string amount, Document& json, AccountUnitBinance2& unit){
...
void TDEngineBinance2::req_transfer_history(const LFTransferHistoryField* data, int account_index, int requestId, bool flag)
{
    ...
    send_writer->write_frame(data, sizeof(LFTransferHistoryField), source_id, MSG_TYPE_LF_TRANSFER_HISTORY_BINANCE2, 1, requestId);
            ...
            if(d.HasMember("depositList"))
            {
                ...
                for ( int i  = 0 ; i < len ; i++ ) {
                    ...
                    if(ticker.length() > 0) {
                        ...
                        his.Status = d["depositList"].GetArray()[i]["status"].GetInt();
                        ...
                        if(d["depositList"].GetArray()[i].HasMember("addressTag"))
            ...
            if(d.HasMember("withdrawList"))
            {
                ...
                for ( int i  = 0 ; i < len ; i++ ) {
                    ...
                    if(ticker.length() > 0) {
                        ...
                        his.Status = d["withdrawList"].GetArray()[i]["status"].GetInt();
                        ...
                        if(d["withdrawList"].GetArray()[i].HasMember("addressTag"))
    ...
    if(errorId != 0)
    {
        raw_writer->write_error_frame(&his, sizeof(LFTransferHistoryField), source_id, MSG_TYPE_LF_TRANSFER_HISTORY_BINANCE2, 1, requestId, errorId, errorMsg.c_str());
    }
}

void TDEngineBinance2::get_deposit_history(AccountUnitBinance2& unit, Document &json, string asset ,
...
void TDEngineBinance2::get_withdraw_history(AccountUnitBinance2& unit, Document &json, string asset,
                  int status, string startTime, string endTime )
{
...
void TDEngineBinance2::set_reader_thread()
{
    ITDEngine::set_reader_thread();

    KF_LOG_INFO(logger, "[set_reader_thread] rest_thread start on AccountUnitBinance2::loop");
    rest_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinance2::loop, this)));

    KF_LOG_INFO(logger,"[set_reader_thread] cancle_thread start on AccountUnitBinance2::caloop");
    cancel_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinance2::caloop,this)));

   // KF_LOG_INFO(logger,"[set_reader_thread] send_thread start on AccountUnitBinance2::seloop");
  //  send_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinance2::seloop,this)));

    // //仿照上面在这里创建新线程，loop是创建的线程里的主函数
    // KF_LOG_INFO(logger,"[set_reader_thread] rest_thread start on AccountUnitBinance2::testUTC");
    // test_thread = ThreadPtr(new std::thread(boost::bind(&TDEngineBinance2::testUTC,this)));
}

void TDEngineBinance2::loop()
{
...
void TDEngineBinance2::caloop()
{
    ...
    while(isRunning)
    {
        ...
        int errorId = 0;
        std::string errorMsg = "";
        std::lock_guard<std::mutex> lck(cancel_mutex);
        for( it = mapCancelOrder.begin(); it != mapCancelOrder.end(); )
        {
            ...
            {
                AccountUnitBinance2& unit = it->second.unit;
                ...
                if( account_type == "margin")
                {
                    get_margin_order(unit, ticker.c_str(), 0, data.OrderRef, json);
                }
                else
                {
                ...
                else if(json.HasMember("status"))
                {
                    ...
                    if(GetOrderStatus(status) != LF_CHAR_Canceled){
                        ...
                        raw_writer->write_error_frame(&data, sizeof(LFOrderActionField), source_id, MSG_TYPE_LF_ORDER_ACTION_BINANCE2, 1, it->second.request_id, errorId, errorMsg.c_str());
                    }
                    else
                    {
...
void TDEngineBinance2::req_inner_transfer(const LFTransferField* data, int account_index, int requestId)
{
    AccountUnitBinance2& unit = account_units[account_index];
    KF_LOG_DEBUG(logger, "[req_inner_transfer]" << " (rid) " << requestId
                                              ...
                                              << " (Currency) " << data->Currency
                                              << " (Volume) " << data->Volume );
    send_writer->write_frame(data, sizeof(LFTransferField), source_id, MSG_TYPE_LF_INNER_TRANSFER_BINANCE2, 1, requestId);
    int errorId = 0;
    std::string errorMsg = "";
    Document json;
    std::string type = "";
    ...
    if(Currency.length() == 0) {
        ...
        raw_writer->write_error_frame(data, sizeof(LFTransferField), source_id, MSG_TYPE_LF_INNER_TRANSFER_BINANCE2, 1, requestId, errorId, errorMsg.c_str());
        return;
    }
    if ( !strcmp(data->From, "master-main") &&!strcmp(data->To, "master-margin"))
    {
        type = "1";
        response = inner_transfer(Currency,std::to_string(((double)(data->Volume)/scale_offset)),type,json,unit);
    }
    ...
    else if( (!strcmp(data->From, "sub-main") &&!strcmp(data->To, "sub-main")) || (!strcmp(data->From, "master-main") &&!strcmp(data->To, "sub-main")) || (!strcmp(data->From, "sub-main") &&!strcmp(data->To, "master-main")))
    {
        response = sub_inner_transfer(data->FromName, data->ToName, Currency, std::to_string(((double)(data->Volume)/scale_offset)), json, unit);
    }
    if (json.HasParseError() || !json.IsObject()){
        ...
        on_rsp_transfer(data,requestId,errorId,errorMsg.c_str());
        raw_writer->write_error_frame(data, sizeof(LFTransferField), source_id, MSG_TYPE_LF_INNER_TRANSFER_BINANCE2, 1, 
            requestId, errorId, errorMsg.c_str());
        return;
    }
    if(json.HasMember("success") && json["success"].GetBool()){
...
cpr::Response TDEngineBinance2::inner_transfer(string asset, string amount, string type, Document& json, AccountUnitBinance2& unit){
...
cpr::Response TDEngineBinance2::sub_inner_transfer(string fromEmail, string toEmail, string asset, string amount, Document& json, AccountUnitBinance2& unit){
...
std::vector<std::string> TDEngineBinance2::split(std::string str, std::string token)
{
...
void TDEngineBinance2::send_margin_order(AccountUnitBinance2& unit, const char *symbol,
                const char *side,
                const char *type,
...
void TDEngineBinance2::get_margin_order(AccountUnitBinance2& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json)
{
...
void TDEngineBinance2::get_margin_open_orders(AccountUnitBinance2& unit, const char *symbol, Document &json)
{
...
void TDEngineBinance2::cancel_margin_order(AccountUnitBinance2& unit, const char *symbol,
...
void TDEngineBinance2::send_order(AccountUnitBinance2& unit, const char *symbol,
                const char *side,
                const char *type,
                const char *timeInForce,
                double quantity,
 ...
 * */
bool TDEngineBinance2::shouldRetry(int http_status_code, std::string errorMsg, std::string text)
{
...
bool TDEngineBinance2::order_count_over_limit(AccountUnitBinance2& unit)
{
...
void TDEngineBinance2::handle_request_weight(AccountUnitBinance2& unit,RequestWeightType type)
{
...
void TDEngineBinance2::meet_429(AccountUnitBinance2& unit)
{
...
bool TDEngineBinance2::isHandling(AccountUnitBinance2& unit)
{
    std::lock_guard<std::mutex> guard_mutex(*unit.mutex_handle_429);
    uint64_t timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
void TDEngineBinance2::get_order(AccountUnitBinance2& unit, const char *symbol, long orderId, const char *origClientOrderId, Document& json)
{
...
void TDEngineBinance2::cancel_order(AccountUnitBinance2& unit, const char *symbol,
                  long orderId, const char *origClientOrderId, const char *newClientOrderId, Document &json)
{
...
void TDEngineBinance2::get_my_trades(AccountUnitBinance2& unit, const char *symbol, int limit, int64_t fromId, Document &json)
{
...
void TDEngineBinance2::get_open_orders(AccountUnitBinance2& unit, const char *symbol, Document &json)
{
...
void TDEngineBinance2::get_exchange_time(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_exchange_infos(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_account(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_margin_account(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_sub_account(string email, AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_listen_key(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::put_listen_key(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::get_margin_listen_key(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::put_margin_listen_key(AccountUnitBinance2& unit, Document &json)
{
...
void TDEngineBinance2::printResponse(const Document& d)
{
...
void TDEngineBinance2::getResponse(int http_status_code, std::string responseText, std::string errorMsg, Document& json)
{
    json.Parse(responseText.c_str());
}

int64_t TDEngineBinance2::getTimestamp()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
    return timestamp;
}

std::string TDEngineBinance2::getTimestampString()
{
    long long timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
int64_t TDEngineBinance2::getTimeDiffOfExchange(AccountUnitBinance2& unit)
{
...
void TDEngineBinance2::on_lws_connection_error(struct lws* conn)
{
    KF_LOG_ERROR(logger, "TDEngineBinance2::on_lws_connection_error.");
    AccountUnitBinance2& unit = findAccountUnitByWebsocketConn(conn);
    ...
    if(lws_login(unit, timeout_nsec))
    {
        ...
        while(it != unit.ordersMap.end() )
        {
            ...
            if( account_type == "margin")
            {
                get_margin_order(unit, ticker.c_str(), remoteOrderId, data.OrderRef, json);
            }
            else
            {
...
int TDEngineBinance2::lws_write_subscribe(struct lws* conn)
{
    return 0;
}

bool TDEngineBinance2::lws_login(AccountUnitBinance2& unit, long timeout_nsec) {
    KF_LOG_INFO(logger, "TDEngineBinance2::lws_login:");
    global_td = this;
    //
    int errorId = 0;
    string errorMsg = "";
    ...
    if (unit.context == NULL) {
        ...
        KF_LOG_INFO(logger, "TDEngineBinance2::lws_login: context created:"<< unit.api_key);
    }

    if (unit.context == NULL) {
        KF_LOG_ERROR(logger, "TDEngineBinance2::lws_login: context of" << unit.api_key <<" is NULL. return");
        return false;
    }
    ...
    KF_LOG_INFO(logger, "TDEngineBinance2::lws_login: Connecting to " <<  ccinfo.host << ":" << ccinfo.port << ":" << ccinfo.path);

    if (unit.websocketConn == NULL) {
        KF_LOG_ERROR(logger, "TDEngineBinance2::lws_login: wsi create error.");
        return false;
    }
    unit.last_put_time = getTimestamp();
    KF_LOG_INFO(logger, "TDEngineBinance2::lws_login: wsi create success.");
    return true;
}


void TDEngineBinance2::on_lws_data(struct lws* conn, const char* data, size_t len) {
    AccountUnitBinance2 &unit = findAccountUnitByWebsocketConn(conn);
    KF_LOG_INFO(logger, "TDEngineBinance2::on_lws_data: " << data);
    Document json;
    json.Parse(data,len);
    if (json.HasParseError() || !json.IsObject()) {
        KF_LOG_ERROR(logger, "TDEngineBinance2::on_lws_data. parse json error: " << data);        
    }
	else if(json.HasMember("e"))
	{
        ...
        {
            KF_LOG_INFO(logger, "TDEngineBinance2::on_lws_data. Order Update ");
            AccountUnitBinance2& unit= findAccountUnitByWebsocketConn(conn);
            std::lock_guard<std::mutex> lck(*unit.mutex_order_and_trade);
            onOrder(unit,json);
        }
        ...
        {
            KF_LOG_INFO(logger, "TDEngineBinance2::on_lws_data. Account Update ");
        }
...
AccountUnitBinance2& TDEngineBinance2::findAccountUnitByWebsocketConn(struct lws * websocketConn)
{
    for (size_t idx = 0; idx < account_units.size(); idx++) {
        AccountUnitBinance2 &unit = account_units[idx];
...
//cys add
std::string TDEngineBinance2::parseJsonToString(Document &d){
    StringBuffer buffer;
    Writer<StringBuffer> writer(buffer);
    d.Accept(writer);
...
*/
void TDEngineBinance2::onOrder(AccountUnitBinance2& unit, Document& json) {
	KF_LOG_INFO(logger, "TDEngineBinance2::onOrder");
    if (json.HasMember("c")&& json.HasMember("i") && json.HasMember("X")&& json.HasMember("l")&& json.HasMember("L")&& json.HasMember("z")&& json.HasMember("t")) {
		...
		if (it == unit.ordersMap.end())
		{ 
			KF_LOG_ERROR(logger, "TDEngineBinance2::onOrder,no order match,save wsOrderStatus");
        ...
        {
            KF_LOG_INFO(logger, "TDEngineBinance2::onOrder,status is not changed");
            return;
        }
		...
		KF_LOG_INFO(logger, "TDEngineBinance2::onOrder,rtn_order");
		on_rtn_order(&rtn_order);
		raw_writer->write_frame(&rtn_order, sizeof(LFRtnOrderField),source_id, MSG_TYPE_LF_RTN_ORDER_BINANCE2,1, (rtn_order.RequestID > 0) ? rtn_order.RequestID : -1);
		...
		KF_LOG_INFO(logger, "TDEngineBinance2::onOrder,rtn_trade");
        if(oldVolumeTraded != rtn_order.VolumeTraded){
		    on_rtn_trade(&rtn_trade);
		    raw_writer->write_frame(&rtn_trade, sizeof(LFRtnTradeField),source_id, MSG_TYPE_LF_RTN_TRADE_BINANCE2, 1, -1);
        }
         //---------------成交总量+1------------------------
...
BOOST_PYTHON_MODULE(libbinance2td)
{
    using namespace boost::python;
    class_<TDEngineBinance2, boost::shared_ptr<TDEngineBinance2> >("Engine")
    .def(init<>())
    .def("init", &TDEngineBinance2::initialize)
    .def("start", &TDEngineBinance2::start)
    .def("stop", &TDEngineBinance2::stop)
    .def("logout", &TDEngineBinance2::logout)
    .def("wait_for_stop", &TDEngineBinance2::wait_for_stop);
}
