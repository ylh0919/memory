...
void TDEngineBittrex::req_withdraw_currency(const LFWithdrawField* data, int account_index, int requestId){
    AccountUnitBittrex& unit = account_units[account_index];
...
void TDEngineBittrex::req_transfer_history(const LFTransferHistoryField* data, int account_index, int requestId, bool flag)
{
    ...
    if(!flag)
    {
        get_deposit_history(unit, d, currency);
        KF_LOG_INFO(logger, "[req_transfer_history] req_deposit_history");
    }
    else
    {
        get_withdraw_history(unit, d, currency);
    ...
    send_writer->write_frame(data, sizeof(LFTransferHistoryField), source_id, MSG_TYPE_LF_TRANSFER_HISTORY_BITTREX, 1, requestId);
/*
deposit
{
  "Id": 1,
    ...
    */
    ...
    if(!d.HasParseError() && d.IsObject())
    {
        if(d.HasMember("result"))
        {
            ...
            for ( int i  = 0 ; i < len ; i++ ) {
                char Currency[32];
                strncpy(Currency, d["result"].GetArray()[i]["Currency"].GetString(), 32 );
                std::string ticker = unit.positionWhiteList.GetKeyByValue(Currency);
                if(ticker.length() > 0) {
                    strncpy(his.Currency, Currency, 32);
                    his.Volume = std::round(d["result"].GetArray()[i]["Amount"].GetDouble() * scale_offset);
                    strncpy(his.TxId, d["result"].GetArray()[i]["TxId"].GetString(), 130);
                    if(d["result"].GetArray()[i].HasMember("addressTag"))
                    strncpy(his.Tag, d["result"].GetArray()[i]["addressTag"].GetString(), 64);
                    if(flag)                             //withdraw
                    {
                        strncpy(his.TimeStamp, (d["result"].GetArray()[i]["Opened"].GetString()), 32);
                        strncpy(his.Address, d["result"].GetArray()[i]["Address"].GetString(), 130);
                        tmp_vector.push_back(his);
                        KF_LOG_INFO(logger,  "[req_withdraw_history] (Opened)" << his.TimeStamp << " (Amount)" <<  his.Volume
                                             << " (Currency)" << his.Currency<< " (Address)" << his.Address <<" (addressTag)" <<his.Tag
                                             << " (TxId)" << his.TxId <<" (requestId)" << requestId );
                    }
                    else                                 //deposit
                    {
                        strncpy(his.TimeStamp, (d["result"].GetArray()[i]["LastUpdated"].GetString()), 32);
                        strncpy(his.Address, d["result"].GetArray()[i]["CryptoAddress"].GetString(), 130);
                        tmp_vector.push_back(his);
                        KF_LOG_INFO(logger,  "[req_deposit_history] (LastUpdated)" << his.TimeStamp << " (Amount)" <<  his.Volume
                                             << " (Currency)" << his.Currency<< " (CryptoAddress)" << his.Address <<" (addressTag)" <<his.Tag
                                             << " (TxId)" << his.TxId <<" (requestId)" << requestId );
                    }
                }
            }
    ...
    if(errorId != 0)
    {
        raw_writer->write_error_frame(&his, sizeof(LFTransferHistoryField), source_id, MSG_TYPE_LF_TRANSFER_HISTORY_BITTREX, 1, requestId, errorId, errorMsg.c_str());
    }
}
...
void TDEngineBittrex::get_deposit_history(AccountUnitBittrex& unit, Document &json, string currency )
{
    string path = "/account/getdeposithistory";
    string postData = "";
    if(currency != "" ){
        postData = postData + "currency="+currency;
    }
    KF_LOG_INFO(logger, "[get_deposit_history] (currency) " << currency );
    int64_t nonce = getTimestamp();
    string nonceStr=std::to_string(nonce);
    KF_LOG_INFO(logger,"[get_deposit_history] (nonce) "<<nonceStr);
    if(postData == ""){
        postData=postData+"apikey="+unit.api_key+"&nonce="+nonceStr;
    }else{
        postData=postData+"&apikey="+unit.api_key+"&nonce="+nonceStr;
    ...
    string message = unit.baseUrl+path+"?"+postData;
...
void TDEngineBittrex::get_withdraw_history(AccountUnitBittrex& unit, Document &json, string currency )
{
    string path = "/account/getwithdrawalhistory";
    string postData = "";
    if(currency != ""){
        postData = postData + "currency="+currency;
    }
    KF_LOG_INFO(logger, "[get_withdraw_history] (currency) " << currency);
    int64_t nonce = getTimestamp();
    string nonceStr=std::to_string(nonce);
    KF_LOG_INFO(logger,"[get_withdraw_history] (nonce) "<<nonceStr);
    if(postData == ""){
        postData=postData+"apikey="+unit.api_key+"&nonce="+nonceStr;
    }else{
        postData=postData+"&apikey="+unit.api_key+"&nonce="+nonceStr;
