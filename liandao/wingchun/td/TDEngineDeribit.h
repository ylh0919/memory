#ifndef PROJECT_TDENGINEDERIBIT_H
#define PROJECT_TDENGINEDERIBIT_H
 ...
 * CTP trade engine
 */
        class TDEngineDeribit: public ITDEngine
        {
        public:
            ...
            virtual string name() const { return "TDEngineDeribit"; };
        ...
        private:
            ...
            vector<AccountUnitDeribit> account_units;

            std::string GetSide(const LfDirectionType& input);
            LfDirectionType GetDirection(std::string input);
            std::string GetType(const LfOrderPriceTypeType& input);
            LfOrderStatusType GetOrderSatus(const std::string& input);
            ...
            //void handlerResponseOrderStatus(AccountUnitDeribit& unit, std::vector<PendingOrderStatus>::iterator orderStatusIterator, ResponsedOrderStatus& responsedOrderStatus);
            void addResponsedOrderStatusNoOrderRef(ResponsedOrderStatus &responsedOrderStatus, Document& json);
            void getPriceIncrement(AccountUnitDeribit& unit);
            void dealPriceVolume(AccountUnitDeribit& unit,const std::string& symbol,int64_t nPrice,int64_t nVolume,double& dDealPrice,double& dDealVome,bool Isbuy);
        ...
        private:
            void get_account(AccountUnitDeribit& unit, Document& json);
            void get_auth(AccountUnitDeribit& unit, Document& json);
            void send_buyorder(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price);
            void send_sellorder(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price);

            void get_order();
            void get_response();
            void get_account_summary();
            void get_state(string orderid);
            void cancel_all_orders();
            void cancel_order(std::string orderId);
            void printResponse(const Document& d);

            std::string createbuyInsertOrderString(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price);
            std::string createsellInsertOrderString(const char *code,const char* strClientId,const char *side, const char *type, double& size, double& price);
            std::string createCancelOrderString(const char* strOrderId = nullptr);
            std::string createCancelallOrderString(const char* strOrderId = nullptr);
            std::string createstateString(string orderid);
            std::string createaccountString();
            std::string createorderString();
            std::string createresponseString();
        public:
            void writeErrorLog(std::string strError);
            void on_lws_data(struct lws* conn, const char* data, size_t len);
            void onOrderChange( Document& d);
            void onCancel( Document& d);
            void onNewOrder( Document& d);
            void onTradeChannel( Document& msg);
            void onerror(Document& msg);
            int lws_write_msg(struct lws* conn);
            void on_lws_connection_error(struct lws* conn);
        private:     
            std::string makeSubscribeChannelString(AccountUnitDeribit& unit);
            std::string makeSubscribeorderString(AccountUnitDeribit& unit);
            std::string makeSubscribetradeString(AccountUnitDeribit& unit);
            std::string makeheartbeatString(AccountUnitDeribit& unit);
            std::string sign(const AccountUnitDeribit& unit,const std::string& method,const std::string& timestamp,const std::string& endpoint);
        ...
        private:
            ...
            bool m_isSuborderOK = false; 
            bool m_isSubtradeOK = false; 
            bool orderfinish=false;
            bool tradefinish=false; 
            bool heartfinish=false;
...
#endif //PROJECT_TDENGINEDERIBIT_H
