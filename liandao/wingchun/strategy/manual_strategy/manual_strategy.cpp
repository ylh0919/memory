...
using exchange_map_t = std::unordered_map<std::string, int>;
...
struct order_info
{
	...
	int exch_source = SOURCE_UNKNOWN;		
...
struct manual_strategy_controller_context
{
	...
	std::unordered_map<int, ticker_market_data_t> all_ticker_market_data;
	...
	std::unordered_map<int, std::unordered_set<int>> exchange_order_info;
...
public:
    ...
    void on_market_data(const LFMarketDataField* data, int source, long rcv_time)
	{
	...
	void on_l2_trade(const LFL2TradeField* data, int source, long rcv_time)
	{
		std::lock_guard<std::mutex> guard(mscc.market_data_mutex);
	...
	void on_rtn_order(const LFRtnOrderField* msg, int request_id, int source, long rcv_time)
	{
...
public:
    ...
    virtual void on_rsp_order(const LFInputOrderField* data, int request_id, int source, long rcv_time, int errorId, const char* errorMsg);
    virtual void on_rsp_order_action(const LFOrderActionField* data, int request_id, int source, long rcv_time, int errorId, const char* errorMsg);
...
void ManualStrategy::on_rsp_position(const PosHandlerPtr posMap, int request_id, int source, long rcv_time)
{
    if (request_id == -1)
    {
...
void ManualStrategy::on_market_data(const LFMarketDataField* md, int source, long rcv_time)
{
	msc.on_market_data(md, source, rcv_time);
}

void ManualStrategy::on_l2_trade(const LFL2TradeField* data, int source, long rcv_time)
{
	msc.on_l2_trade(data, source, rcv_time);
}

void ManualStrategy::on_rtn_order(const LFRtnOrderField* msg, int request_id, int source, long rcv_time)
{
...
void ManualStrategy::on_rsp_order_action(const LFOrderActionField* data, int request_id, int source, long rcv_time, int errorId, const char* errorMsg)
{
    if (errorId != 0)
	{
...
void ManualStrategy::on_rtn_trade(const LFRtnTradeField* rtn_trade, int request_id, int source, long rcv_time)
{
...
void ManualStrategy::on_rsp_order(const LFInputOrderField* order, int request_id, int source, long rcv_time, int errorId, const char* errorMsg)
{
    if (errorId != 0)
	{
        KF_LOG_ERROR(logger, " (err_id)" << errorId << " (err_msg)" << errorMsg << "(order_id)" << request_id << " (source)" << source);
