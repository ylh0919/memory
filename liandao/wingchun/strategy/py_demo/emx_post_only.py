...
def initialize(context):
    ...
    context.ticker = 'btc_u19'
    context.exchange_id = EXCHANGE.SHFE
    context.buy_price = -1
    context.sell_price = -1
    context.order_rid = -1
    context.cancel_id = -1
    context.add_td(source=SOURCE.EMX)
    context.subscribe(tickers=[context.ticker], source=SOURCE.EMX)
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.EMX)
            ...
            context.buy_price = 900000000000 #market_data.LowerLimitPrice
            context.sell_price = 999999999 #market_data.UpperLimitPrice
            if context.order_rid < 0:
                print("context.insert_limit_order 1.")
                context.order_rid = context.insert_limit_order(source=SOURCE.EMX,
                                                               ticker=context.ticker,
                                                               price=context.buy_price,
                                                               exchange_id=context.exchange_id,
                                                               volume=2300000,
                                                               direction=DIRECTION.Buy,
                                                               offset=OFFSET.Open,misc_info="0000000000000000000000000000000000000001")
