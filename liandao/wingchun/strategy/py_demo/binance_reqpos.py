'''
wingchun strategy -n my_test -p /liandao/wingchun/strategy/py_demo/binance_reqpos.py
'''
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            #context.stop()
            print '----will test reqpos----'
            #context. = 44100 #market_data.LowerLimitPrice
            #context.sell_price = 20 #market_data.UpperLimitPrice
            
            print("context.reqpos 1.")
            context.order_rid = context.req_pos(source=SOURCE.BINANCE,
                                        account_type = "master-margin",
                                        account_name = "")
