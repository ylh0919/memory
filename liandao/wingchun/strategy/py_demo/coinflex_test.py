...
def initialize(context):
    ...
    context.subscribe(tickers=[context.ticker], source=SOURCE.COINFLEX)
    print('init ok')
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            context.req_pos(source=SOURCE.COINFLEX)
            ...
            context.buy_price = 16112345678 #market_data.LowerLimitPrice
            context.sell_price = 999999999 #market_data.UpperLimitPrice
            if context.order_rid < 0:
                print("context.insert_limit_order 1.")
                context.order_rid = context.insert_limit_order(source=SOURCE.COINFLEX,
