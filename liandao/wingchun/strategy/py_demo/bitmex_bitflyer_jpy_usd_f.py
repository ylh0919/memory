...
bitmex_BidLevelCount = 0
bitmex_AskLevelCount = 0
D = []
I = []
N = []
Q = []
...
def writefile1(timea):
    global yest,path,path_bitflyer,path_bitmex,path_jpy_usd
    timea /= 10**9
    time_local = time.localtime(int(timea))     
    dt = time.strftime("%Y-%m-%d",time_local)
    if yest != dt:
        yest = dt
        path = "/shared/jpy_usd_bitmex/"+dt
        if not os.path.exists(path):
            os.makedirs(path)
        else:
            path_bitflyer = path+'/md_bitflyer.csv'
            path_jpy_usd = path+'/jpy_usd_bitmex.csv'
            path_bitmex = path+'/md_bitmex.csv'
            return
        path_bitflyer = path+'/md_bitflyer.csv'
        path_jpy_usd = path+'/jpy_usd_bitmex.csv'
        path_bitmex = path+'/md_bitmex.csv'
        ...
        with open(path_jpy_usd,'w') as f:
            ...
            csv_head = ["InstrumentID","bitmex_time","bitflyer_time","bitmex_nano","bitflyer_nano"]
            for i in range(0,20):
        ...
        with open(path_bitmex,'w') as f:
            csv_write = csv.writer(f)
            csv_head = ["InstrumentID","bitmex_time","bitmex_nano","BidLevelCount","AskLevelCount"]
            for i in range(0,20):
                csv_head.append("BidVol["+str(i)+"]")
                csv_head.append("BidPrice["+str(i)+"]")
            for i in range(0,20):
...
def initialize(context):
    ...
    context.subscribe(tickers=['btc_usd'], source=SOURCE.BITMEX)
...
def on_price_book(context, price_book, source, rcv_time):
    if source != SOURCE.BITMEX and source != SOURCE.BITFLYER:
        return
    global path_bitflyer,path_bitmex,path_jpy_usd
    global bitflyer_data,bitflyer_inverse_data,bitmex_data,bitmex_time,bitflyer_time,D,I,N,Q,V,W,X,Y,Z,AC,path,bitflyer_BidLevelCount,bitflyer_AskLevelCount,bitmex_BidLevelCount,bitmex_AskLevelCount
        ...
        if price_book.InstrumentID == "fx_btc_jpy":
            ...
            if bitmex_data != []:
                ...
                for i in range(0,20):
                    ...
                    X.append('%.f'%(10**8*float(bitmex_data[1+temp_a*2])*float(bitflyer_inverse_data[1+temp_b*2])))
                ...
                for i in range(0,20):
                    ...
                    Y.append('%.f'%(10**8*float(bitmex_data[40+temp_c*2])*float(bitflyer_inverse_data[40+temp_d*2])))
                    Z.append('%.f'%(10**8*askusdvolume/float(bitflyer_inverse_data[40+temp_d*2])))
                
                writefile1(bitmex_time)
                with open(path_jpy_usd,'a+') as f:
                    csv_write = csv.writer(f)
                    data_row = ["jpy_usd"]
                    data_row.append(timestamp_tran_time(bitmex_time))
                    data_row.append(timestamp_tran_time(bitflyer_time))
                    data_row.append(bitmex_time)
                    ...
                    print("bitmex_bitflyer_jpy_usd_f")
                    print(data_row)

                with open(path_bitmex,'a+') as f:
                    csv_write = csv.writer(f)
                    data_row = ["btc_usd"]
                    data_row.append(timestamp_tran_time(bitmex_time))
                    data_row.append(bitmex_time)
                    data_row.append(bitmex_BidLevelCount)
                    data_row.append(bitmex_AskLevelCount)
                    for i in range(0,80):
                        data_row.append(bitmex_data[i])
                    csv_write.writerow(data_row)
            ...
            I = []
            bitmex_data = []
            bitmex_time = rcv_time
            bitmex_BidLevelCount = price_book.BidLevelCount
            bitmex_AskLevelCount = price_book.AskLevelCount
            for i in range(0,price_book.BidLevelCount):
                bitmex_data.append(float(price_book.BidLevels.levels[i].volume)/10**8)
                bitmex_data.append(float(price_book.BidLevels.levels[i].price)/10**8)
                D.append(float(price_book.BidLevels.levels[i].volume)/10**8)
            for i in range(price_book.BidLevelCount,20):
                bitmex_data.append(0)
                bitmex_data.append(0)
                D.append(0)
            for i in range(0,price_book.AskLevelCount):
                bitmex_data.append(float(price_book.AskLevels.levels[i].price)/10**8)
                bitmex_data.append(float(price_book.AskLevels.levels[i].volume)/10**8)
