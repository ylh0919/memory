'''
wingchun strategy -n my_test -p /liandao/wingchun/strategy/py_demo/bitfinex_transfer_history.py
'''
...
def on_pos(context, pos_handler, request_id, source, rcv_time):
    ...
    if request_id == -1:
        if pos_handler is None:
            ...
            #context.stop()
            print '----will test transfer history----'
            #context. = 44100 #market_data.LowerLimitPrice
            #context.sell_price = 20 #market_data.UpperLimitPrice
            if context.order_rid < 0:
                print("context.transfer history 1.")
                context.order_rid = context.transfer_history(source=SOURCE.BITFINEX,
                                                             flag=True,                   #flase for deposit,true for withdraw
                                                             curreny="BTC",
                                                             status= -1,
                                                             start_Time="",
                                                             end_Time="",
...
def on_transfer_history(context, transfer_history, request_id, source, rcv_time, is_Last, flag):
    if flag is False:
        print '----on deposit history----'
    else:
        print '----on withdraw history----'
    if is_Last is True:
        print '---------the end-----------'
    else:
