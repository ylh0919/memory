...
kumex_BidLevelCount = 0
kumex_AskLevelCount = 0
D = []
I = []
N = []
Q = []
...
def writefile1(timea):
    global yest,path,path_bitflyer,path_kumex,path_jpy_usd
    timea /= 10**9
    time_local = time.localtime(int(timea))     
    dt = time.strftime("%Y-%m-%d",time_local)
    if yest != dt:
        yest = dt
        path = "/shared/jpy_usd_kumex/"+dt
        if not os.path.exists(path):
            os.makedirs(path)
        else:
            path_bitflyer = path+'/md_bitflyer.csv'
            path_jpy_usd = path+'/jpy_usd_kumex.csv'
            path_kumex = path+'/md_kumex.csv'
            return
        path_bitflyer = path+'/md_bitflyer.csv'
        path_jpy_usd = path+'/jpy_usd_kumex.csv'
        path_kumex = path+'/md_kumex.csv'
        ...
        with open(path_jpy_usd,'w') as f:
            ...
            csv_head = ["InstrumentID","kumex_time","bitflyer_time","kumex_nano","bitflyer_nano"]
            for i in range(0,20):
        ...
        with open(path_kumex,'w') as f:
            csv_write = csv.writer(f)
            csv_head = ["InstrumentID","kumex_time","kumex_nano","BidLevelCount","AskLevelCount"]
            for i in range(0,20):
                csv_head.append("BidVol["+str(i)+"]")
                csv_head.append("BidPrice["+str(i)+"]")
            for i in range(0,20):
...
def initialize(context):
    ...
    context.add_md(source=SOURCE.KUMEX)
    context.add_md(source=SOURCE.BITFLYER)
    # context.add_td(source=SOURCE.KUMEX)
    # context.add_td(source=SOURCE.BITFLYER)
    context.subscribe(tickers=['btc_usd'], source=SOURCE.KUMEX)
...
def on_price_book(context, price_book, source, rcv_time):
    if source != SOURCE.KUMEX and source != SOURCE.BITFLYER:
        return
    global path_bitflyer,path_kumex,path_jpy_usd
    global bitflyer_data,bitflyer_inverse_data,kumex_data,kumex_time,bitflyer_time,D,I,N,Q,V,W,X,Y,Z,AC,path,bitflyer_BidLevelCount,bitflyer_AskLevelCount,kumex_BidLevelCount,kumex_AskLevelCount
        ...
        if price_book.InstrumentID == "fx_btc_jpy":
            ...
            if kumex_data != []:
                ...
                for i in range(0,20):
                    ...
                    X.append('%.f'%(10**8*float(kumex_data[1+temp_a*2])*float(bitflyer_inverse_data[1+temp_b*2])))
                ...
                for i in range(0,20):
                    ...
                    Y.append('%.f'%(10**8*float(kumex_data[40+temp_c*2])*float(bitflyer_inverse_data[40+temp_d*2])))
                    Z.append('%.f'%(10**8*askusdvolume/float(bitflyer_inverse_data[40+temp_d*2])))
                
                writefile1(kumex_time)
                with open(path_jpy_usd,'a+') as f:
                    csv_write = csv.writer(f)
                    data_row = ["jpy_usd"]
                    data_row.append(timestamp_tran_time(kumex_time))
                    data_row.append(timestamp_tran_time(bitflyer_time))
                    data_row.append(kumex_time)
                    ...
                    print("kumex_bitflyer_jpy_usd_f")
                    print(data_row)

                with open(path_kumex,'a+') as f:
                    csv_write = csv.writer(f)
                    data_row = ["btc_usd"]
                    data_row.append(timestamp_tran_time(kumex_time))
                    data_row.append(kumex_time)
                    data_row.append(kumex_BidLevelCount)
                    data_row.append(kumex_AskLevelCount)
                    for i in range(0,80):
                        data_row.append(kumex_data[i])
                    csv_write.writerow(data_row)
            ...
            kumex_data = []
            kumex_time = rcv_time
            kumex_BidLevelCount = price_book.BidLevelCount
            kumex_AskLevelCount = price_book.AskLevelCount
            for i in range(0,price_book.BidLevelCount):
                kumex_data.append(float(price_book.BidLevels.levels[i].volume)/10**8)
                kumex_data.append(float(price_book.BidLevels.levels[i].price)/10**8)
            ...
            for i in range(price_book.BidLevelCount,20):
                ...
                D.append(0)
            for i in range(0,price_book.AskLevelCount):
                kumex_data.append(float(price_book.AskLevels.levels[i].price)/10**8)
                kumex_data.append(float(price_book.AskLevels.levels[i].volume)/10**8)
