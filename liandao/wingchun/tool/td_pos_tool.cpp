...
bool set_pos_tool(const string& user_name, int source, const string& file_name, bool is_csv)
{
...
bool get_pos_tool(const string& user_name, int source, bool print_raw, bool need_detail, bool print_file, bool is_csv, const string& output_file)
{
            ...
            if (boost::filesystem::is_regular_file(itr->status()))
            {
                ...
                if (idx > 0)
                {
                    try
                    {
                        int source;
                        string td_name;
                        source = std::stoi(filename.substr(idx + 1));
