...
int ClientPageProvider::register_journal(const string& dir, const string& jname)
{
    ...
    std::string rs = std::to_string((int)rsp->type);
    std::string re = std::to_string((int)req.type);
    std::string suc = std::to_string(rsp->success);
    ...
    if (rsp->type == req.type && rsp->success)
        ...
        throw std::runtime_error("cannot register journal: " + client_name + " rsp->type:" + rs + " req.type:" + re + " rsp->success:" + suc);
...
bool StrategySocketHandler::td_connect(int source)
{
    SocketMArray rspArray;
    PagedSocketRequest req = {};
...
bool StrategySocketHandler::md_subscribe(const vector<string>& tickers, int source, int msg_type)
{
    size_t idx = 0;
