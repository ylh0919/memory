...
context_usage = \
    [   # utilities
        ...
        ('get_name', lambda strategy: strategy.get_name,
         ...
         ('str', [])),
         ('is_running', lambda strategy: strategy.is_running,
         'check  strategy running',
         ('bool', [])),
        ...
        ('add_md', lambda strategy: strategy.get_data_wrapper().add_market_data,
         ...
         (None, [('int', 'source')])),
        ('set_timerange', lambda strategy: strategy.get_data_wrapper().set_time_range,
         'add startnano and endnano in \"set_time_range\" function',
         (None, [('long', 'startnano'), ('long', 'endnano')])),
        ...
        ('req_pos', lambda strategy: strategy.req_position,
         ...
         ('int', [('int', 'source'),('str','account_type'),('str','account_name')])),
        ('cancel_order', lambda strategy: strategy.cancel_order,
         'cancel order by order_id, return request_id (-1 if failed)',
         ('int', [('int', 'source'), ('int', 'order_id'), ('str', 'misc_info')])),
         ('batch_cancel_order', lambda strategy: strategy.batch_cancel_order,
         'batch cancel order by order_id_list, return request_id (-1 if failed)',
         ('int', [('int', 'source'), ('int*', 'order_id_list'), ('str*', 'misc_info_list'), ('int', 'size_of_list')])),
         ('req_get_kline_via_rest', lambda strategy: strategy.req_get_kline_via_rest,
         'get kline via rest, from td',
         ('int', [('int', 'source'), ('str', 'ticker'), ('str', 'Interval'), ('int', 'Limit'), ('bool', 'IgnoreStartTime'), ('int', 'start_Time'), ('int', 'end_Time')])),
        # position
        ...
        ('insert_fak_order', lambda strategy: strategy.insert_fak_order,
         ...
         ('int', [('int', 'source'), ('str', 'ticker'), ('str', 'exchange_id'), ('int', 'price'), ('int', 'volume'), ('DIRECTION', 'direction'), ('OFFSET', 'offset'), ('str', 'misc_info')])),
         #rfq
        ('insert_quote_request', lambda strategy: strategy.insert_quote_request,
         'insert qoute request, return quote request id (-1 if failed)',
         ('int', [('int', 'source'), ('str', 'ticker'), ('str', 'exchange_id'), ('str', 'expiry'),('int', 'volume'), ('DIRECTION', 'direction'), ('int', 'price'), ('bool', 'is_hide_limit_price'), ('str', 'misc_info')])),
        ('cancel_quote_request', lambda strategy: strategy.cancel_quote_request,
         'cancel qoute request by quote request id (-1 if failed)',
         ('int', [('int', 'source'), ('int', 'quote_request_id'), ('str', 'misc_info')])),

        ('insert_quote', lambda strategy: strategy.insert_quote,
         'insert qoute, return quote id (-1 if failed)',
         ('int', [('int', 'source'), ('str', 'ticker'), ('int', 'quote_request_id'), ('int', 'price'), ('str', 'misc_info')])),
        
        ('cancel_quote', lambda strategy: strategy.cancel_quote,
         'cancel qoute  by quote id (-1 if failed)',
         ('int', [('int', 'source'), ('int', 'quote_id'), ('str', 'misc_info')])),

        ('accept_quote', lambda strategy: strategy.accept_quote,
         'accept qoute  by quote id (-1 if failed)',
         ('int', [('int', 'source'), ('int', 'quote_id'), ('str', 'misc_info')])),
        ...
        ('log_fatal', lambda strategy: lambda x: strategy.log_fatal(str(x)),
         ...
         (None, [('str', 'msg')])),
        # errormsg
        ('write_errormsg', lambda strategy: strategy.write_errormsg,
         'write error message',
         ('int', [('int', 'source'), ('int', 'errorId'), ('str', 'errorMsg')])),
        # withdraw
        ('withdraw_currency', lambda strategy: strategy.withdraw_currency,
         'withdraw currency',
         ('int', [('int', 'source'), ('str', 'currency'), ('int', 'volume'), ('str', 'address'),('str', 'tag')])),
        ('req_inner_transfer', lambda strategy: strategy.req_inner_transfer,
         'innser transfer',
         ('int', [('int', 'source'), ('str', 'currency'), ('int', 'volume'), ('str', 'from_type'),('str', 'to_type'),('str','from_name'),('str','to_name'),('str','ticker')])),
        # transfer_history
        ('transfer_history', lambda strategy: strategy.transfer_history,
         'transfer_history',
         ('int', [('int', 'source'), ('bool','flag'),('str', 'currency'),('int', 'status'),('str', 'start_Time'),('str', 'end_Time'),('str', 'from_id')])),
        #ws_url
        ('set_ws_url', lambda strategy: strategy.set_ws_url,
         "set monitor's url",
         (None, [('str', 'ws_url')])),
    ]
...
override_methods = [
    ...
    ('on_ticker', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('ticker_data', 'LFTicker', 'ticker data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.')]),
      'callback when tick data received', False),
    ('on_quote_requests', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('quote_request_data', 'LFQuoteRequestsField', 'quote request data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.')]),
     'callback when ticker data received', False),
    ('on_priceindex', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('priceindex_data', 'LFPriceIndex', 'priceindex data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.')]),
    'callback when ticker data received', False),
    ('on_markprice', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('markprice_data', 'LFMarkPrice', 'markprice data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.')]),
    'callback when ticker data received', False),
    ('on_perpetual', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('perpetual_data', 'LFPerpetual', 'perpetual data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.')]),
    'callback when ticker data received', False),
    ...
    ('on_withdraw', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('withdraw', 'LFWithdrawField', 'withdraw data'),
                        ('order_id', 'int', 'order id'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of withdraw info received.')]),
     'callback when withdraw data received', False),
    ('on_transfer', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('transfer', 'LFTransferField', 'transfer data'),
                        ('order_id', 'int', 'order id'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of inner transfer info received.')]),
     'callback when withdraw data received', False),
    ('on_transfer_history', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('transfer_history', 'LFTransferHistoryField', 'transfer_history data'),
                        ('request_id', 'int', 'order id'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of tick received.'),
                        ('is_Last', 'bool', 'judge end'),
                        ('flag', 'bool', 'judge deposit or withdraw')]),
     'callback when transfer history data received', False),
    ('on_bar_data', (None, [('context', 'context object', CONTEXT_COMMENT),
                        ('bar_data', 'LFBarMarketDataField', 'bar data'),
                        ('source', 'int', SOURCE_COMMENT),
                        ('rcv_time', 'long', 'time of bar data received.')]),
     'callback when bar data received', False),
    ...
    ('on_rtn_quote', (None, [('context', 'context object', CONTEXT_COMMENT),
                             ('rtn_quote', 'LFRtnQuoteField', 'return-quote data'),
                             ('req_id', 'int', 'request id of quote'),
                             ('source', 'int', SOURCE_COMMENT),
                             ('rcv_time', 'long', 'time of rtn_order received.')]),
      'callback when quote data received', False),
    ('on_bar_serial1000', (None, [('context', 'context object', CONTEXT_COMMENT),
                             ('bar_serial_data', 'LFBarSerial1000Field', 'bar serial_data'),
                             ('request_id', 'int', 'request_id'),
                             ('source', 'int', SOURCE_COMMENT),
                             ('rcv_time', 'long', 'time of rtn_trade received.')]),
     'callback when bar serial from restful api(get bar via rest) is received', False),
]
class_details = [
    ('PosHandler', [], [('add_pos', (None, [('str', 'ticker'),
                        ...
                        ('get_account_type', ('str', []), 'get account type'),
                        ('get_account_name', ('str', []), 'get account name'),
                        ],
