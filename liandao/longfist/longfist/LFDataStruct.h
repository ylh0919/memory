...
{
    char_32      TradeTime;             //成交时间（2015-07-07T10:44:33.547Z）
    char_16      ExchangeID;            //交易所代码
    char_31      InstrumentID;          //合约代码
    int64_t      Price;                 //成交价格
    ...
    int64_t      IndexPrice;
    int64_t      MarkPrice;
    int64_t      TimeStamp;                //时间戳(1553846281766256031)
    bool         Liquidation;            //true if the trade involved a liquidation order
};
...
struct LFBarMarketDataField
{
    ...
    uint64_t   	 CurrencyVolume;        //The trading volume in a specific token
    //bool         IsComplete;            //这根K线是否完结(是否已经开始下一根K线)
};

struct LFQuoteRequestsField
{
    char_32      InstrumentID;          //合约代码
    char_16      ExchangeID;            //交易所代码
    char_32      Expiry;                    //when the Instrument expires
    int64_t      ID;
    int64_t      Price;                 
    uint64_t     Volume; 
    char_2       OrderBSFlag;
    char_32      RequestExpiry;            //when the request expires
    char_32      Time;                    //when the request was placed
    char_9       Status;
};

struct LFQryPositionField
{
    char_32      BrokerID;              //经纪公司代码
    char_32      InvestorID;            //投资者代码
    char_31      InstrumentID;          //合约代码
    char_16       ExchangeID;            //交易所代码
};
...
struct LFInputOrderField
{
    ...
    char_16                        ExchangeID;            //交易所代码
    char_31                        InstrumentID;          //合约代码
    char_32                        Expiry;                //合约到期时间 2020-10-10T10:00:00Z,ISO8601
    ...
    char_32                        OrderExpiry;           //订单到期时间 2020-10-10T10:00:00Z,ISO8601
};
...
struct LFTransferField{
    ...
    char_32     Currency;
    int64_t     Volume;
    char_32     Symbol;       //huobi
    char_64     ID;
};
enum LFTransferStatus
{
    TRANSFER_STRATUS_PROCESSING,
    TRANSFER_STRATUS_SUCCESS,
    TRANSFER_STRATUS_CANCELED,
    TRANSFER_STRATUS_FAILURE,
    TRANSFER_STRATUS_REJECTED
};
struct LFTransferHistoryField
{
    bool                 IsWithdraw;               // withraw/deposit
    char_64              UserID;                //用户代码
    char_16             ExchangeID;            //交易所代码
    int                 Status;                //状态   0.....6....
    char_32              TimeStamp;             //发生时间
    int64_t             Volume;                //数量
    char_130            Address;               //地址
    char_32             Currency;              //币种
    char_64             Tag;       
...
struct LFTicker
{
    ...
    double        Vega;
    double        Volume24;
    char_31       Underlying_index;
};

struct LFInputQuoteField
{
    char_32                        InstrumentID;          //合约代码
    char_21                        OrderRef;              //报单引用
    char_16                        ExchangeID;            //交易所代码
    int64_t                        Price;                 //价格
    int64_t                        QuoteRequestID;        //quote request id
};
struct LFQuoteActionField
{
    char_32                        InstrumentID;          //合约代码
    int64_t                        QuoteID;                    //quote id
    char_21                        OrderRef;
    int                            RequestID;
    int                            KfOrderID;             //Kf系统内订单ID
    char                           ActionFlag;            //报单操作标志, 0 cancel Quote ,1  accept quote
};
struct LFRtnQuoteField
{
    int64_t                        ID;                    //quote id
    char_32                        InstrumentID;          //合约代码
    char_21                        OrderRef;              //报单引用
...
struct LFBatchCancelOrderField          //只有td_binancef才使用
{
    char_11      BrokerID;              //经纪公司代码
    char_19      InvestorID;            //投资者代码
    char_31      InstrumentID;          //合约代码
    char_16      ExchangeID;            //交易所代码
    char_16      UserID;                //用户代码
    int          SizeOfList;            //一次性撤单的总个数 最大5
    int          RequestID;             //请求编号
    BatchOrderInfoField  InfoList[5];
};
...
struct GetKlineViaRest 
{
    char_16      ExchangeID;            //交易所代码
    char_31      Symbol;                //交易对,InstrumentID
    char_16      Interval;              //时间间隔
    bool         IgnoreStartTime;       //是否设置开始和结束时间
    int64_t      StartTime;             //起始时间，时间戳
    int64_t      EndTime;               //结束时间，时间戳
    int          Limit;                 //请求数量
    int          RequestID;
    char_64 MiscInfo;                   //委托自定义标签
};
...
struct LFSingleBar
{
    int64_t     StartUpdateMillisec;   //开盘时间
    int64_t     EndUpdateMillisec;     //收盘时间
    int         PeriodMillisec;        //周期（毫秒）
    ...
    int64_t       High;                  //高
    uint64_t      Volume;                //成交量
    uint64_t      BusinessVolume;        //成交额
    uint64_t      ActInVolume;           //主动买入成交量
    uint64_t      ActInBusinessVolume;   //主动买入成交额
    int           TransactionsNum;       //成交笔数
};

template<int level_count>
struct LFBarSerialTmpl
{
    char_31      InstrumentID;             //合约代码
    char_16      ExchangeID;               //交易所代码
    int          BarLevel;
    LFSingleBar  BarSerial[level_count];
};

using LFBarSerial1000Field = LFBarSerialTmpl<1000>;
